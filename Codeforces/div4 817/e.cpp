...
void solve()
{
    ...
    vpll narr;
    fl(i, 0, n) narr.push_back({rect[i][0], rect[i][1]});

    while (q--)
    {
        int hs, ws, hb, wb;
        cin >> hs >> ws >> hb >> wb;
        pll c1 = {hs, ws}, c2 = {hb, wb};
        int indx1 = upper_bound(vr(narr), c1) - narr.begin();
        int indx2 = upper_bound(vr(narr), c2) - narr.begin();
        if (indx1 != indx2)
        {
            ll ans = 0;
            if (indx2 == n)
