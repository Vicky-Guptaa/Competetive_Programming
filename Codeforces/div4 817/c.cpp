...
void solve()
{
    ...
    fl(i, 0, 3)
    {
        fl(j, 0, n)
        {
            if (s1.count(words[i][j]) && s2.count(words[i][j]) && s3.count(words[i][j]))
                continue;

            if ((s1.count(words[i][j]) && s2.count(words[i][j])) || (s2.count(words[i][j]) && s3.count(words[i][j])) ||
                (s1.count(words[i][j]) && s3.count(words[i][j])))
            {
                score[i]++;
                continue;
            }
