...
void solve()
{
    ...
    cin >> arr;
    k--;
    ll left = k - 1, right = k + 1, sum = arr[k];
    while (true)
    {
        ll tsum = sum;
        ll itr = left;
        ...
        if (maxi == sum && maxj == sum)
        {
            pn return;
        }
        else if (maxi != sum && maxj != sum)
            if (maxj > maxi)
            {
                sum = maxj;
                right = maxjndx;
            ...
            left = maxindx, sum = maxi;
        else
            right = maxjndx, sum = maxj;
    }
    pn return;
}
/*
