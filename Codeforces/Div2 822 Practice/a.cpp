...
void solve()
{
    ...
    for (int i = 0; i < freq.size(); i++)
    {
        ...
        {
            if (i != 0)
                minOper = min(minOper, abs(freq[i].first - freq[i - 1].first));
            if (i != (int)freq.size() - 1)
                minOper = min(minOper, abs(freq[i + 1].first - freq[i].first));
        }
        else
        {
            if (i != 0 && i != (int)freq.size() - 1)
            {
                minOper = min(minOper, abs(freq[i + 1].first - freq[i - 1].first));
            }
        }
    }
    cout << minOper << "\n";
