...
int helper(int indx, vi &arr, vector<int> list[], vector<int> &dp)
{
    if (indx + 1 >= dp.size())
        return 0;
    ...
    int strt = upper_bound(list[arr[indx]].begin(), list[arr[indx]].end(), indx) - list[arr[indx]].begin();
    // cout<<strt<<"\n";

    dp[indx] = helper(indx + 1, arr, list, dp);

    for (int i = strt; i <= list[arr[indx]].size(); i++)
    {
        dp[indx] = max(dp[indx], helper(list[arr[indx]][i] + 1, arr, list, dp) + (list[arr[indx]][i] - indx + 1));
    }
    return dp[indx];
}
