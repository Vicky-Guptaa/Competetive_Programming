...
void solve()
{
    ...
    fl(i, freq.size()) mset.insert(0);

    if (freq.size() == 1 || freq.size() == n)
    {
        py return;
    }
    ...
    fl(i, freq.size())
    {
        ...
        int val = freq[s[i]];
        mset.insert(val);
        mset.erase(mset.find(val - 1));
    }

    if (*mset.rbegin() - *mset.begin() > 1)
    {
        pn return;
    }

    for (int i = freq.size(); i < n; i++)
    {
        freq[s[i]]++;
        int val = freq[s[i]];
        mset.insert(val);
