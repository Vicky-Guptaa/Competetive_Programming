...
string solve1(vll &arr, int n, set<ll> &oset)
{
    fl(i, 0, n)
    {
        fl(j, i + 1, n)
        {
            fl(k, j + 1, n)
            {
                if (!oset.count(arr[i] + arr[j] + arr[k]))
                    return "No";
            }
        }
    }
...
string solve2(vll &arr, int n, set<ll> &oset)
{
    mll freq;
    fl(i, 0, n) freq[arr[i]]++;
    if (freq.count(0) && freq[0] >= n - 2)
    {
        if (freq[0] == n || freq[0] == n - 1)
        {
            return "Yes";
        }
        else if (freq[0] == n - 2 && freq.begin()->first + freq.rbegin()->first == 0)
        {
            return "Yes";
        }
    }
...
void solve()
{
    ...
    while (true && cntr < 20)
    {
        ...
        sort(vr(arr));
        set<ll> oset(vr(arr));
        string ans1 = solve1(arr, n, oset);
        string ans2 = solve2(arr, n, oset);
        cout << n << "\n";
        cout << arr << "\n";
        cout << ans1 << " " << ans2 << "\n";
        if (ans1 != ans2)
