...
ll sumAnswer(vector<vll> &mat, int r, int c)
{
    ll sum = mat[r][c];
    int _r = r - 1, _c = c - 1;
    while (_r >= 0 && _c >= 0)
    ...
    _r = r - 1;
    _c = c + 1;
    while (_r >= 0 && _c < mat[0].size())
        sum += mat[_r--][_c++];

    _r = r + 1;
    _c = c - 1;
    while (_r < mat.size() && _c >= 0)
        sum += mat[_r++][_c--];

    _r = r + 1;
    _c = c + 1;
    while (_r < mat.size() && _c < mat[0].size())
        sum += mat[_r++][_c++];
...
void solve()
{
    ...
    fl(i, n)
    {
        fl(j, m)
        {
            maxSum = max(maxSum, sumAnswer(mat, i, j));
        }
    }
    cout << maxSum << "\n";
}
