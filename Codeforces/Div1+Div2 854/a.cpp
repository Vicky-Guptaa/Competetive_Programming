...
void solve()
{
    ...
    for (int i = 0; i < m; i++)
    {
        if (mpVal.count(arr[i]))
        {
            space.erase(mpVal[arr[i]]);
            space[number] = arr[i];
            mpVal[arr[i]] = number--;
        }
        else
        {
            mpVal[arr[i]] = number--;
            mpVal.erase(space.rbegin()->second);
            if (space.rbegin()->second <= n)
            {
                crr[space.rbegin()->second] = time;
            }
            space.erase(space.rbegin()->first);
            space[mpVal[arr[i]]] = arr[i];
        }
        time++;
    }
    fl(i, 1, n + 1)
