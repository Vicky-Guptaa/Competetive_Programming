...
void solve()
{
    ...
    while (true)
    {
        ...
        int val = ceil(mx / (lld)mn);
        int indxmn = *freq.begin()->second.begin();
        int indxmx = *freq.rbegin()->second.begin();
        brr.push_back({indxmx + 1, indxmn + 1});
        freq.rbegin()->second.erase(indxmx);
        if (freq.rbegin()->second.empty())
        {
            freq.erase(freq.rbegin()->first);
        }
        freq[val].insert(indxmx);
        if (brr.size() > 30 * n)
        {
            break;
        }
