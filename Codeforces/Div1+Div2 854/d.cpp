...
ll helper(ll strt, ll prevCpu, ll prevK, vi &arr, vi &brr, vi &crr, vector<vector<vll>> &dp)
{
    if (strt == arr.size())
        return 0;
    if (dp[strt][prevK][prevCpu] != -1)
        return dp[strt][prevK][prevCpu];

    ll costCpu1 = 0;
    ll costCpu2 = 0;
    if (prevCpu == 0 && (strt > 0 && arr[strt - 1] == arr[strt]))
    {
        costCpu1 = crr[arr[strt] - 1];
        costCpu2 = brr[arr[strt] - 1];
    }
    else
    {
        costCpu1 = brr[arr[strt] - 1];
        if (prevK == arr[strt])
            costCpu2 = crr[arr[strt] - 1];
        else
            costCpu2 = brr[arr[strt] - 1];
    }
    dp[strt][prevK][prevCpu] = min(helper(strt + 1, 0, prevK, arr, brr, crr, dp) + costCpu1,
                                   costCpu2 + helper(strt + 1, 1, (strt > 0) ? arr[strt - 1] : 0, arr, brr, crr, dp));
    costCpu1 = 0;
    costCpu2 = 0;
    if (prevCpu == 1 && (strt > 0 && arr[strt - 1] == arr[strt]))
    {
        costCpu2 = crr[arr[strt] - 1];
        costCpu1 = brr[arr[strt] - 1];
    }
    ...
    return dp[strt][prevK][prevCpu] = min({helper(strt + 1, 1, prevK, arr, brr, crr, dp) + costCpu2,
                                           costCpu1 + helper(strt + 1, 0, (strt > 0) ? arr[strt - 1] : 0, arr, brr, crr, dp), dp[strt][prevK][prevCpu]});
}
...
void solve()
{
    ll n, k;
    cin >> n >> k;
    vi arr(n), brr(k), crr(k);
    cin >> arr >> brr >> crr;
    vector<vector<vll>> dp(n + 1, vector<vll>(k + 1, vector<ll>(2, -1)));
