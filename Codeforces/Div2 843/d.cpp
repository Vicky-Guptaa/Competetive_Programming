...
Vickys-Air:Div2 843 vicky_gupta$ cd "/Users/vicky_gupta/Desktop/code/Coding/Competetive_Programming/Codeforces/Div2 843/" && g++ -std=c++17 d.cpp -o d && "/Users/vicky_gupta/Desktop/code/Coding/Competetive_Programming/Codeforces/Div2 843/"d
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    ll src, tar, srcIndx, tarIndx;
    cin >> src >> tar;
    vector<int> seive = Sieve();
    int s = 3e5 + 10;
    set<int> slist[s];
    vector<int> list[s];
    fl(i, 0, n)
    {
        ...
        while (num != 1)
        {
            if (!slist[seive[num]].count(i))
                slist[seive[num]].insert(i);
            num /= seive[num];
        }
    }
    fl(i, 0, s)
    ...
    {
        if (!visit[seive[src]])
            que.push({seive[src], srcIndx});
        src /= seive[src];
        visit[seive[src]] = true;
    }
    vector<int> path(n + 1, -1);
    path[srcIndx] = srcIndx;
    ...
    while (!que.empty())
    {
        ...
        for (auto x : list[curr])
        {
            ...
            while (num != 1)
            {
                if (!visit[seive[num]])
                {
                    visit[seive[num]] = true;
                    que.push({seive[num], x});
                }
                num /= seive[num];
            }
            if (arr[x] == tar)
    ...
    {
        vector<int> result;
        int currIndx = tarIndx;
        while (path[currIndx] != currIndx)
        {
            result.push_back(currIndx);
            currIndx = path[currIndx];
        }
        result.push_back(currIndx);
        cout << result << "\n";
    }
