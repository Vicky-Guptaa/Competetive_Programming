...
ll helper(int strt, int num, int k, vll &arr, vector<vi> &dp)
{
    if (strt == arr.size())
    {
        if (num == -1)
            return 0;
        return __builtin_popcount(num) == k;
    }

    if (dp[strt][num + 1] != -1)
        return dp[strt][num + 1];

    if (num == -1)
    {
        return dp[strt][num + 1] = (helper(strt + 1, arr[strt], k, arr, dp) + (ll)helper(strt + 1, num, k, arr, dp)) % mod;
    }
    // if (__builtin_popcount(num) < k)
    //     return 0;
    return dp[strt][num + 1] = (helper(strt + 1, num, k, arr, dp) + (ll)helper(strt + 1, (num & arr[strt]), k, arr, dp)) % mod;
}
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    vector<vi> dp(n, vi(70, -1));
    cout << helper(0, -1, k, arr, dp) << "\n";
}
/*
