...
ll helper(int s, bool isShift, string &str, vll &arr, vector<vll> &dp)
{
    if (s == str.size())
        return 0;
    if (dp[s][isShift] != -1)
    ...
    {
        if (isShift)
            return dp[s][isShift] = max(helper(s + 1, true, str, arr, dp) + arr[s - 1], helper(s + 1, false, str, arr, dp) + arr[s]);
        else
        {
            if (s == 0 || str[s - 1] == '1')
            {
                return dp[s][isShift] = helper(s + 1, false, str, arr, dp) + arr[s];
            }
            else
            {
                return dp[s][isShift] = max(helper(s + 1, true, str, arr, dp) + arr[s - 1], helper(s + 1, false, str, arr, dp) + arr[s]);
    ...
    {
        return dp[s][isShift] = helper(s + 1, false, str, arr, dp);
    }
}
