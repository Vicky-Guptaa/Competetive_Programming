...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    map<ll, set<int>> flr, frr;
    flr[arr[0]].insert(0);
    fl(i, 1, n) frr[arr[i] - (i)].insert(i);
    ll ans = 0;
    fl(i, 1, n - 1)
    {
        ll curr = arr[i] - (i);
        ll indx = *frr[curr].begin();
        frr[curr].erase(frr[curr].begin());
        if (frr[curr].empty())
            frr.erase(curr);
        ll temp = flr.rbegin()->first + frr.rbegin()->first + arr[i];
        temp += i - indx;
        ans = max(ans, temp);
        flr[arr[i] + i].insert(i);
    }
    cout << ans << "\n";
}
/*
