...
void dfs(int src, ll val, vpi list[], vector<bool> &visited, set<ll> &oset)
{
    visited[src] = true;
    oset.insert(val);
    for (auto [v, w] : list[src])
    {
        if (!visited[v])
        {
            dfs(v, val ^ w, list, visited, oset);
        }
    }
}
// Code
void solve()
{
    ...
    visited[b] = true;
    dfs(a, 0, list, visited, oset);
    fl(i, 0, n + 1) visited[i] = false;
    queue<pll> que;
    que.push({b, 0});
    visited[b] = true;
    while (!que.empty())
    {
        auto [curr, val] = que.front();
        que.pop();
        for (auto [v, w] : list[curr])
        {
            if (!visited[v])
            {
                visited[curr] = true;
