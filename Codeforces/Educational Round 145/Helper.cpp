...
ll solve1(string s)
{
    ...
    for (int a = i; a <= j; a++)
    {
        ...
        if (s[a] == '1' && a < n - 1 && s[a + 1] == '0')
        {

            ll curr = (po - 1) * sd + min((z - pz - 1) * sd, (o - po + 1) * sd) + sw;
            res = min((ll)res, (ll)curr);
        }
    ...
    //         else z++;
    //     }
    //     cout<<min(res,min(sd*z,sd*o)+score)<<"\n";
    // }
}
