...
int search(vll &preSum, ll ele, ll sub, int i)
{
    if (preSum.back() - sub <= ele)
    {
        return (int)preSum.size() - 1;
    }
    ll low = i, high = (int)preSum.size() - 1, ans = i;
    while (low <= high)
    {
        ll mid = (low + high) / 2;
        if (preSum[mid] - sub <= ele)
        {
            ans = mid;
            low = mid + 1;
        }
...
void solve()
{
    ll n;
    cin >> n;
    vll arr(n), brr(n), fullContri(n, 0), preSum(n, 0), partialContri(n, 0);
    cin >> arr >> brr;
    ...
    fl(i, 0, n)
    {
        ll prev = (i == 0) ? 0 : preSum[i - 1];
        ll indx = search(preSum, arr[i], prev, i);
        if (indx == i)
        {
            if (arr[i] > brr[i])
            {
                ...
                {
                    fullContri[i + 1]--;
                    partialContri[i + 1] += min(brr[i + 1], arr[i] - brr[i]);
                }
            }
            else
            {
                partialContri[i] += min(brr[i], arr[i]);
            }
        }
        else
        {
            ...
            if (indx + 1 < n && preSum[indx] - prev < arr[i])
            {
                partialContri[indx + 1] += min(brr[indx + 1], arr[i] - (preSum[indx] - prev));
            }
        }
    }
    fl(i, 1, n)
    {
        fullContri[i] += fullContri[i - 1];
    }
    fl(i, 0, n)
    {
        partialContri[i] += fullContri[i] * brr[i];
    }
    cout << partialContri << "\n";
}
/*
