...
vll costSum(vll arr)
{
    ...
    fl(i, 1, n - 1)
    {
        ...
        {
            costArr[i] = max(arr[i - 1], arr[i + 1]) - arr[i] + 1;
        }
        i++;
    }
    fl(i, 1, n)
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    vll cost1 = costSum(arr);
    reverse(vr(arr));
    vll cost2 = costSum(arr);
    reverse(vr(cost2));
    if (n & 1)
    {
        cout << cost1.back() << "\n";
    ...
    else
    {
        ...
        fl(i, 1, n - 1)
        {
            ans = min(cost1[i] + cost2[i + 2], ans);
            i++;
        }
        rl(i, n - 2, 1)
        {
            ans = min(cost2[i] + cost1[i - 2], ans);
            i--;
        }
        cout << ans << "\n";
    }
