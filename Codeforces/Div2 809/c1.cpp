...
ll helper(int s, vll &arr, vll &dp)
{
    if (s >= arr.size() - 1)
        return 0;
    ...
    {
        return dp[s] = helper(s + 2, arr, dp);
    }
    else
    {
        return dp[s] = min(helper(s + 1, arr, dp), max(arr[s - 1], arr[s + 1]) - arr[s] + 1 + helper(s + 2, arr, dp));
    }
}
