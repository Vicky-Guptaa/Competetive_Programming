...
void solve()
{
    ...
    ll iter = 1;
    ll extraCost = 0, maxRemC = 0, cnt = 0, newCost = 1e15;
    for (auto x : freq)
    {
        if (x.first == iter)
        {
        ...
        {
            newCost = min({newCost, cost + ((ll)freq.size() - cnt) * c + extraCost});
            extraCost += (x.first - iter) * d;
            maxRemC = max(maxRemC, ((ll)freq.size() - cnt) * c);
            if (extraCost > maxRemC)
            {
                break;
            }
            iter = x.first + 1;
