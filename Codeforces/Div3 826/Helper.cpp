...
int helper(int x, int y, int red, int n, int m, int target, vector<vector<vector<int>>> &dp)
{
    if (red > target || x + y - red > target)
        return -1;
    if (x >= n || y >= m)
        return -1;
    ...
    ll ans = 0;
    ans += helper(x + 1, y, red + 1, n, m, target, dp) + 1;
    ans += helper(x, y + 1, red + 1, n, m, target, dp) + 1;
    ans += helper(x + 1, y, red, n, m, target, dp) + 1;
    ans += helper(x, y + 1, red, n, m, target, dp) + 1;
    return dp[x][y][red] = ans;
}
...
void solve()
{
    ...
    else
    {
        ...
        cout << helper(0, 0, 0, n, m, (m + n - 1) / 2, dp) << "\n";
    }
}
/*
