...
int bsearch(vpll &prr, int k)
{
    int low = 0, high = prr.size() - 1;
    int ans = prr.size();
    while (low <= high)
    {
    ...
    if (ans != prr.size() && prr[ans].first <= k)
        ans++;
...
void solve()
{
    ...
    {
        auto ele = prr[i];
        int indx = bsearch(prr, ele.second);
        ans += (int)prr.size() - indx;
    }
    cout << ans << endl;
}
/*
