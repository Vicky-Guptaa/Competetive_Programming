...
ll helper(vpll list[], vll &s)
{
    ...
    while (!pque.empty())
    {
        ...
        ll cycle = pque.top().second.first;
        ll curr = pque.top().second.second;
        pque.pop();
        for (auto &child : list[curr])
        {
            ll ndist = cost + cycle * child.second;
            ll slow = min(cycle, s[child.first - 1]);
            if (dist[child.first][slow] <= ndist)
            {
                continue;
            }
            dist[child.first][slow] = ndist;
            pque.push({ndist, {slow, child.first}});
        }
    }
    for (int i = 0; i < 1001; i++)
    {
        res = min(res, dist[s.size()][i]);
    }
    // cout << dist << "\n";
    return res;
}
