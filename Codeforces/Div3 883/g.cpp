...
void solve()
{
    ...
    fl(i, 0, m)
    {
        ...
        list.push_back({d, {fix, sym}});
    }
    priority_queue<ppi, vpi, greater<ppi>> pque;
    vector<int> dis((1 << n)+1, 1e9);
    pque.push({0, start});
    dis[start] = 0;
    while (!pque.empty())
    {
        int day = pque.top().first;
        int state = pque.top().second;
        pque.pop();
        fl(i, 0, m)
        {
            int newState=((state & list[i].second.first) | list[i].second.second);
            if (dis[newState] > day + list[i].first)
            {
                dis[newState] = day + list[i].first;
                pque.push({dis[newState], newState});
            }
        }
    }
    if (dis[0] == 1e9)
