...
double func(vll &arr, vll &brr, double m)
{
    double cost = 0;
    fl(i, 0, arr.size())
    {
        cost = max(brr[i] + abs(m - arr[i]), cost);
    }
    return cost;
}
...
void solve()
{
    ...
    vll arr(n), brr(n);
    cin >> arr >> brr;
    double l = *min_element(vr(arr)), r = *max_element(vr(arr));
    double eps = 1e-7; // set the error limit here
    while (r - l > eps)
    {
        double m1 = l + (r - l) / 3;
        double m2 = r - (r - l) / 3;
        double f1 = func(arr, brr, m1); // evaluates the function at m1
        double f2 = func(arr, brr, m2); // evaluates the function at m2
        if (f1 > f2)
            l = m1;
        else
            r = m2;
