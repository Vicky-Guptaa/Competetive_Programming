...
int helper(int s, int e, int n, vll &arr, vector<vi> &dp)
{
    if (s == e)
        return 1;
    if (dp[s][e] != -1)
    ...
    if (arr[s] == arr[e])
    {
        return dp[s][e] = max(helper(s + 1, e, n, arr, dp), helper(s, e - 1, n, arr, dp));
    }
    else
    {
        return dp[s][e] = 1 + max(helper(s + 1, e, n, arr, dp), helper(s, e - 1, n, arr, dp));
    }
}
