...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    vll preSum(n + 1, 0), preXr(n + 1, 0);
    fl(i, 1, n + 1) preSum[i] = preSum[i - 1] + arr[i - 1];
    map<int, set<int>> mmpo, mmpe;
    mll mindx;
    ll preXor = 0;
    for (int i = 0; i < n; i++)
    {
        ...
        preXr[i + 1] = preXor;
        if ((i + 1) & 1)
            mmpo[preXor].insert(i + 1);
        else
            mmpe[preXor].insert(i + 1);
    }
    while (q--)
    {
        ll l, r;
        ...
        else
        {
            if ((preXr[l - 1] ^ preXr[r]) == 0)
            {
                ...
                else
                {
                    ...
                    if (l & 1)
                    {
                        auto curr = mmpo[fxor].lower_bound(l);
                        if (curr != mmpo[fxor].end() && *curr < r && *curr > l)
                        {
                            cout << "2\n";
                        }
                        else
                    ...
                    {
                        auto curr = mmpe[fxor].lower_bound(l);
                        if (curr != mmpe[fxor].end() && *curr < r && *curr > l)
                        {
                            cout << "2\n";
                        }
                        else
