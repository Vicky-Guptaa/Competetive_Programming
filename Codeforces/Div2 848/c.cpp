...
void calc(vector<int> &aset, string &a, string &b, ll &ans)
{
    ll cntr = 0, temp = 0;
    fl(i, 0, a.size())
    {
        if (a[i] == b[i] || (aset[a[i] - 'a']))
        {
            cntr++;
        }
        else
...
void helper(int s, int k, string &a, string &b, vector<char> &crr, vector<int> &aset, ll &ans)
{
    int n = crr.size();
    if (s == n || k == 0)
    {
    ...
    aset[crr[s] - 'a'] = 1;
    helper(s + 1, k - 1, a, b, crr, aset, ans);
    aset[crr[s] - 'a'] = 0;
    helper(s + 1, k, a, b, crr, aset, ans);
}
...
void solve()
{
    ...
    vector<int> aset(26, 0);
    
    if (k > sset.size())
        k = sset.size();

    helper(0, k, a, b, crr, aset, ans);
    cout << ans << "\n";
}
/*
