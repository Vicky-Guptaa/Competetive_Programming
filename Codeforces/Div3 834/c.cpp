...
// Code
int helper(ll l, ll r, ll x, ll a, ll b, bool isFind = true)
{
    if (a == b)
    {
        return 0;
    }
    if ((a + x > r && a - x < l) || b > r || b < l)
    {
        return -1;
    }
    if (abs(a - b) >= x)
    ...
    if (isFind)
    {
        if (a + x <= r && helper(l, r, x, a + x, b, false) == 2)
        {
            return 3;
        }
        if (a - x >= l && helper(l, r, x, a - x, b, false) == 2)
        {
            return 3;
        }
    }
