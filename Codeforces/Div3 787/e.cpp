...
void solve()
{
    ...
    {
        auto find = omap.lower_bound(s[i]);
        if (find != omap.end())
        {
            omap[find->first].insert(i);
        }
        else
        {
            omap[s[i]].insert(i);
    ...
    for (auto x : omap)
    {
        if (k > x.first - prev)
        {
            corres[x.first] = {x.first - prev, prev};
            k -= x.first - prev;
        }
        else
        {
            corres[x.first] = {k, prev};
            k -= x.first - prev;
            break;
        }
        iter++;
    ...
    if (k >= 0 && iter == omap.size())
    {
        fl(i, n) cout << 'a';
        cout << "\n";
        return;
    }
    auto x = corres.rbegin();
    ll smallest = *omap[x->first].begin();
    ll res = x->first - x->second.first;
    prev = x->second.second;
    for (auto y : omap[x->first])
    {
        if (res < s[y])
        {
            s[y] = char(res);
