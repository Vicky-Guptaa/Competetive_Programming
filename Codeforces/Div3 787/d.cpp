...
void dfs(int src, vll list[], vll &ans, vector<vll> &result, vll &isVisited)
{
    isVisited[src] = true;
    ans.push_back(src);
    for (auto x : list[src])
    {
        ...
        {
            dfs(x, list, ans, result, isVisited);
        }
    }
}

void dfs_leaf(int src, vll list[], vll &leaf)
{
    for (auto x : list[src])
    {
        if (x != src)
        {
            dfs_leaf(x, list, leaf);
        }
    }
    if (list[src].empty())
    {
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        list1[i + 1].push_back(arr[i]);
        list2[arr[i]].push_back(i + 1);
    }
    vll leaf;
    dfs_leaf(src, list2, leaf);
    // cout << leaf << endl;
    vll isVisited(n + 2, false);
    vector<vll> result;
    for (auto x : leaf)
    {
        vll ans;
        dfs(x, list1, ans, result, isVisited);
        reverse(vr(ans));
