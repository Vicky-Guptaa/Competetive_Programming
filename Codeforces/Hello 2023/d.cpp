...
vll Nearest_Right(vll &arr, int n)
{
    stack<int> st;
    vll ans(n, 0);
    for (int i = n - 1; i >= 0; i--)
...
void solve()
{
    ...
    map<int, vector<int>> mindx;

    for (int i = 0; i < n; i++)
        mindx[brr[i]].push_back(i);

    vll nge = Nearest_Right(brr, n);
    for (auto x : mindx)
    {
        if (x.first == mindx.rbegin()->first)
            continue;
        for (int i = 0; i < x.second.size(); i++)
        {
            int iter = i;
            if (brr[x.second[i]] == arr[x.second[i]])
                continue;
            while (iter < x.second.size() && nge[x.second[i]] >= x.second[iter])
            {
                iter++;
            }
            cut[x.first]--;
