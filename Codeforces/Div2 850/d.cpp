...
void solve()
{
    ...
    fl(i, 0, n)
    {
        ...
        if (srr[i][0] == srr[i][1])
        {
            findx[srr[i][0]].insert(i);
        }
        else
        {
            findx[srr[i][1]].insert(i);
        }
    }
    if (findx.empty())
    {
    ...
    vector<pair<int, char>> chnge;
    while (findx.count('i'))
    {
        int cindx = *findx['i'].begin();
        chnge.push_back({cindx + 1, 'i'});
        if (srr[cindx][2] == 'i')
        {
            if (findx.count('n'))
            {
                int indx = *findx['n'].begin();
                chnge.push_back({indx + 1, 'n'});
                if (srr[indx][0] == srr[indx][1] && srr[indx][2] == srr[indx][1])
                {
                    srr[indx][0] = 'i';
                }
                else
            ...
            {
                int indx = *findx['w'].begin();
                chnge.push_back({indx + 1, 'w'});
                if (srr[indx][0] == srr[indx][1] && srr[indx][2] == srr[indx][1])
                {
                    srr[indx][0] = 'i';
                }
    ...
    while (findx.count('n'))
    {
        int cindx = *findx['n'].begin();
        chnge.push_back({cindx + 1, 'n'});
        int indx = *findx['w'].begin();
        chnge.push_back({indx + 1, 'w'});
        findx['n'].erase(cindx);
        findx['w'].erase(indx);
