...
vector<vector<int>> calcFactors(vll &arr)
{
    vector<vector<int>> allFact;
    int n = arr.size();
    fl(i, 0, n)
    {
        vi row;
        ...
        sort(vr(row));
        allFact.push_back(row);
    }
    return allFact;
}

bool isValid(ll mid, vector<vi> &arr, int m, vll &brr)
{
    ...
    fl(i, 0, n)
    {
        ...
        while (brr[i] - brr[prev] > mid)
        {
            fl(j, 0, arr[prev].size())
            {
                if (arr[prev][j] > m)
                    break;
                freq[arr[prev][j]]--;
                if (freq[arr[prev][j]] == 0)
                    freq.erase(arr[prev][j]);
            }
            prev++;
        }
        if (freq.size() == m)
...
void solve()
{
    ...
    cin >> arr;
    sort(vr(arr));
    vector<vector<int>> factors = calcFactors(arr);
    ll low = 0, high = arr.back(), ans = -1;
    while (low <= high)
    {
        ll mid = (low + high) / 2;
        if (isValid(mid, factors, m, arr))
        {
            high = mid - 1;
            ans = mid;
        }
...
int main()
{
    ...
    // #endif
    // vector<int> sieve = Prime_Sieve(1e5 + 2);
    You Can Do_It
        ll t;
    cin >> t;
    fl(i, 0, t)
