...
void solve()
{
    ...
    fl(i, 0, n - 1)
    {
        if ((srr[0][i + 1] == 'W' && srr[1][i + 1] == 'B' && srr[0][i] == 'B' && srr[1][i] == 'W') ||
            (srr[0][i + 1] == 'B' && srr[1][i + 1] == 'W' && srr[0][i] == 'W' && srr[1][i] == 'B'))
        {
            pn return;
        }
    }
    fl(i, 0, n)
    {
        if (srr[0][i] == 'B' && srr[1][i] == 'B')
        {
            int cntr = 0, iter = i;
            while (iter < n && srr[0][iter] == 'B' && srr[1][iter] == 'B')
            {
                iter++;
                cntr++;
            }
            ...
            if (i != 0)
            {
                char pupr = srr[0][i - 1];
                char plwr = srr[1][i - 1];
                char nupr = srr[0][iter];
                char nlwr = srr[1][iter];
                if (pupr == nupr && plwr == nlwr)
                {
                    if (cntr & 1)
                    {
                        pn return;
