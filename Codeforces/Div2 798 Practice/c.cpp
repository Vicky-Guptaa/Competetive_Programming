...
ll noOfNode(ll src, vector<int> list[], vpll &hght, vi &visit)
{
    visit[src] = true;
    ll sum = 0;
    vll temp;
    for (auto x : list[src])
    {
        if (!visit[x])
        {
            temp.push_back(noOfNode(x, list, hght, visit));
        }
    }
    visit[src] = false;
    if (temp.size() == 2)
    {
        hght[src] = {temp[0], temp[1]};
    }
    else if (temp.size() == 1)
    {
        hght[src] = {temp[0], 0};
    ...
    return accumulate(vr(temp), 0ll) + 1;
}

ll helper(ll src, vector<int> list[], vpll &hght, vi &visit)
{
    visit[src] = true;
    vll temp;
    for (auto x : list[src])
    ...
    if (temp.empty())
        return 0;
    ll ans1 = hght[src].second + helper(temp[0], list, hght, visit);
    ll ans2 = hght[src].first;
    if (temp.size() > 1)
    {
        ans2 += helper(temp[1], list, hght, visit);
    }
    return max(ans2, ans1) - 1;
}   
...
void solve()
{
    ...
    vpll hght(n + 1);
    vi visit(n + 1, 0);
    noOfNode(1, list, hght, visit);
    ll ans = helper(1, list, hght, visit);
    cout << ans << "\n";
}
/*
