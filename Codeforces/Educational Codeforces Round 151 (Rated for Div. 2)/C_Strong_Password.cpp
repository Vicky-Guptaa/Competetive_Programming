...
int helper(int indx1, int indx2, string &a, string &b, string &s, vector<vector<int>> &dp)
{
    if (indx2 == a.size())
    {
        return false;
    ...
    int strt = a[indx2] - '0', end = b[indx2] - '0';
    fl(i, strt, end + 1)
    {
        int indx = -1;
        fl(j, indx1, s.size())
        ...
        if (indx == -1)
            return true;
        if (helper(indx + 1, indx2 + 1, a, b, s, dp))
        {
            return true;
        }
    }
    return dp[indx1][indx2] = false;
}
