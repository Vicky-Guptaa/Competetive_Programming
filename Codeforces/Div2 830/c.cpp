...
ll calc(vll &preSum, vll &preXor, ll size, ll ans, ll &l, ll &r)
{
    ll n = preSum.size();
    fl(i, size, n)
    {
        if (ans == preSum[i] - preSum[i - size] - (preXor[i] ^ preXor[i - size]))
        {
            l = i - size + 1;
            r = i;
            return true;
...
void solve()
{
    ...
    cin >> l >> r;
    ll low = 1, high = n;
    vll preSum(n + 1, 0), preXor(n + 1, 0);
    fl(i, 1, n + 1)
    {
        preSum[i] = preSum[i - 1] + arr[i - 1];
        preXor[i] = preXor[i - 1] ^ arr[i - 1];
    }
    ll ans = preSum[n] - preXor[n], left = 1, right = n;
    while (low <= high)
    {
        ll mid = (low + high) / 2;
        ll l, r;
        ll ans1 = calc(preSum, preXor, mid, ans, l, r);
        if (ans1)
        {
            high = mid - 1;
            left = l;
