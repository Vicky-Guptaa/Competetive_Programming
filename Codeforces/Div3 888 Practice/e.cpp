...
ll helper(int curr, vector<vll> &mat, vll &arr, vll &brr)
{
    ll cost = arr[curr - 1];
    if (*lower_bound(vr(brr), arr[curr - 1]) == arr[curr - 1])
        return 0;
    ll ncost = 0;
    fl(i, 0, mat[curr - 1].size())
    {
        if (*lower_bound(vr(brr), mat[curr - 1][i]) == mat[curr - 1][i])
            continue;
        ncost += helper(mat[curr - 1][i], mat, arr, brr);
    }
    if (mat[curr - 1].empty())
        return cost;
    return min(cost, ncost);
}
