...
void solve()
{
    ...
    else
    {
        ...
        ll indx = pos[arr.front()][k - 1];
        ll nindx = lower_bound(pos[arr.back()].begin(), pos[arr.back()].end(), indx) - pos[arr.back()].begin();
        fl(i, nindx, pos[arr.back()].size())
        {
            k--;
            if (k <= 0)
                break;
