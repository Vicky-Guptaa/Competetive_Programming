...
vector<int> vvls = {1, 10, 100, 1000, 10000};
ll helper(int s, string &str, int lar, bool isChange, vector<vector<vll>> &dp)
{
    if (s == str.size())
        return 0;

    if (dp[s][lar + 1][isChange] != -1)
        return dp[s][lar + 1][isChange];
    ...
    if (isChange)
    {
        return dp[s][lar + 1][isChange] = helper(s + 1, str, max(lar, str[s] - 'A'), isChange, dp) + val;
    }
    else
    {
        ll mx = helper(s + 1, str, max(lar, str[s] - 'A'), isChange, dp) + val;
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            mx = max(mx, helper(s + 1, str, max(lar, i), true, dp) + val);
        }

        return dp[s][lar + 1][isChange] = mx;
    }
}
...
void solve()
{
    ...
    reverse(vr(s));
    ll n = s.size();
    vector<vector<vector<long long>>> dp(n, vector<vll>(7, vll(2, -1)));
    cout << helper(0, s, -1, 0, dp) << "\n";
}
/*
