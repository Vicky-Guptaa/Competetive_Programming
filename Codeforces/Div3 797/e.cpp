...
void solve()
{
    ...
    fl(i, 0, n)
    {
        if (mp.count((m - Arr[i].first) % m))
        {
            auto itr = *mp.find((m - Arr[i].first) % m);
            ll ans = arr[Arr[i].second] + arr[itr.second];
            ans /= m;
            res += ans;
            mp.erase(mp.find((m - Arr[i].first) % m));
        }
        else
        {
            mp.insert({Arr[i].first, Arr[i].second});
        }
    }
    Arr.clear();
    for (auto x : mp)
    ...
    {
        ll ans = 0;
        if (Arr[i - 1].first + Arr[size].first >= m)
        {
            ans = arr[Arr[i - 1].second] + arr[Arr[size].second];
            size--;
        }
        else
        {
            ans = arr[Arr[i].second] + arr[Arr[i - 1].second];
            i++;
        }
        ans /= m;
        res += ans;
