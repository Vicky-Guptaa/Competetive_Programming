...
void storeHelper(int i, vll &arr, vll &ans)
{
    ll n = arr.size();
    fl(j, 0, i)
    {
...
void storeBig(int i, vll &arr, vll &ans)
{
    ll n = arr.size();
    fl(j, 0, i)
    {
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    ll indx = max_element(vr(arr)) - arr.begin();
    if (indx == 0)
    {
        indx = max_element(arr.begin() + 1, arr.end()) - arr.begin();
    }
    if (indx == n - 1)
    {
        vll ans(n, 0);
