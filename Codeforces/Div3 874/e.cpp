...
bool dfsCycle(ll src, ll par, set<int> list[], vector<int> &visit)
{
    visit[src] = true;
    for (auto child : list[src])
    {
        if (!visit[child])
        {
            if (dfsCycle(child, src, list, visit))
                return true;
        }
        else
        {
...
void solve()
{
    ...
    fl(i, 0, n)
    {
        list[i + 1].insert(arr[i]);
        list[arr[i]].insert(i + 1);
    }
    ...
    // cout << cycle << " " << ncycle << "\n";
    cout << cycle + (ncycle != 0) << " " << cycle + ncycle << "\n";
}
/*
