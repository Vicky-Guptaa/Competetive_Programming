...
void solve()
{
    ...
    cin >> arr;
    sort(vr(arr));
    ll ans = 0, prev = 0, currAns = 1;
    mll freq;
    fl(i, 0, n)
    {
        if (freq.count(arr[i]))
        {
            currAns = division(currAns, freq[arr[i]], mod);
        }
        freq[arr[i]]++;
        ...
        currAns = product(currAns, freq[arr[i]], mod);
        while (freq.rbegin()->first - freq.begin()->first >= m)
        {
            currAns = division(currAns, freq[arr[prev]], mod);
            freq[arr[prev]]--;
            if (!freq[arr[prev]])
            {
                freq.erase(arr[prev]);
            ...
            {
                currAns = product(currAns, freq[arr[prev]], mod);
            }
            prev++;
        }
        if (freq.size() < m)
            continue;
        ans = sum(currAns, ans, mod);
    }
    cout << ans << "\n";
}
/*
