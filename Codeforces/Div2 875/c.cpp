...
void helper(int src, int par, int val, vpi list[], int cnt, ll &res)
{
    res = max(res, (ll)cnt);
    for (auto child : list[src])
    {
        if (child.first != par)
        {
            helper(child.first, src, child.second, list, child.second > val ? cnt : cnt + 1, res);
        }
    }
}
...
void solve()
{
    ...
    fl(i, 0, n - 1)
    {
        ll u, v;
        cin >> u >> v;
        list[u].push_back({v, i+1});
        list[v].push_back({u, i+1});
    }
    ll res = 1;
    helper(1, -1, 0, list, 1, res);
    cout << res << "\n";
}
/*
