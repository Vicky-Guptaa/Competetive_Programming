...
map<int, set<int>> helper(vll &arr)
{
    ...
    fl(i, 1, n)
    {
        ...
        {
            freq[arr[i - 1]].insert(count);
            count = 1;
        }
    }
    freq[arr.back()].insert(count);
    return freq;
}
...
void solve()
{
    ...
    vll arr(n), brr(n);
    cin >> arr >> brr;
    map<int, set<int>> f1 = helper(arr), f2 = helper(brr);
    ll res = 1;
    for (auto x : f1)
    {
        res = max(res, (ll)*x.second.rbegin());
    }
    ...
    for (auto x : f1)
    {
        if (f2.count(x.first))
        {
            res = max(res, (ll)*x.second.rbegin() + (ll)*f2[x.first].rbegin());
        }
    }
    cout << res << "\n";
}
