...
void solve()
{
    ll n;
    cin >> n;
    vi arr(n), brr(n), marr(n + 1), mbrr(n + 1);
    cin >> arr;
    cin >> brr;
    fl(i, 0, n)
    {
    ...
    ll cnt = 1;
    int mn = min(marr[1], mbrr[1]);
    int mx = max(marr[1], mbrr[1]);
    ll left = mn;
    ll right = n - mx - 1;
    ll ans = (left * (left + 1)) / 2;
    ans += (right * (right + 1)) / 2;
    cnt += ans + ((mx - mn - 1ll) * (mx - (ll)mn)) / 2ll;
    // cout << left << " " << right << " ->" << ans << "\n";
    fl(i, 2, n + 1)
    {
        int currmn = min(marr[i], mbrr[i]);
        int currmx = max(marr[i], mbrr[i]);
        ans = 0;
        if (currmx < mn)
        {
            left = mn - currmx;
