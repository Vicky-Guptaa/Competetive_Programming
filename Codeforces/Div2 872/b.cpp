...
void solve()
{
    ...
    cin >> arr;
    sort(vr(arr));
    if (arr[0] == arr[1] || arr[n * m - 1] == arr[n * m - 2])
    {
        ll ans = (n * m - 1) * (arr.back() - arr.front());
        cout << ans << "\n";
    }
    else
    {
        ll d1 = arr[1] - arr[0];
        ll d2 = arr[n * m - 1] - arr[n * m - 2];
        if (n > m)
        {
            if (d1 > d2)
            {
                ll ans = (n * m - m) * (arr[n * m - 1] - arr[0]);
                ans += (m - 1) * (arr[n * m - 2] - arr[0]);
                cout << ans << "\n";
            }
            else
            {
                ll ans = (n * m - m) * (arr[n * m - 1] - arr[0]);
                ans += (m - 1) * (arr[n * m - 1] - arr[1]);
                cout << ans << "\n";
            }
        }
        else
        {
            if (d1 > d2)
            {
                ll ans = (n * m - n) * (arr[n * m - 1] - arr[0]);
                ans += (n - 1) * (arr[n * m - 2] - arr[0]);
                cout << ans << "\n";
            }
            else
            {
                ll ans = (n * m - n) * (arr[n * m - 1] - arr[0]);
                ans += (n - 1) * (arr[n * m - 1] - arr[1]);
                cout << ans << "\n";
            }
        }
    }
