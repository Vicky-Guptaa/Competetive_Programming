...
void solve()
{
    ...
    sort(vr(arr));
    ll res = min(m, (ll)arr.size() + max(left, right));
    fl(i, 0, arr.size())
    {
        ll lspace = arr[i] - i - 1;
        ll rspace = m - arr[i] - ((int)arr.size() - i - 1);
        res = min(m, max(res, (ll)arr.size() + min(lspace, left) + min(rspace, right)));
    }
    cout << res << "\n";
}
/*
