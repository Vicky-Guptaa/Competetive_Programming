...
void solve()
{
    ...
    vll arr(n), narr(n);
    cin >> arr;
    ll mx = *max_element(vr(arr)), mn = *min_element(vr(arr));
    priority_queue<ll> mpque;
    priority_queue<ll, vll, greater<ll>> spque;
    fl(i, 0, n)
    {
        if (arr[i] >= 0)
        {
    ...
    ll sum = 0, diff = mx - mn, iter = 0;
    bool isPos = true;
    while (!spque.empty() || !mpque.empty())
    {
        if (!spque.empty() && !mpque.empty())
        {
            if (isPos)
            {
                ll md = mpque.top();
