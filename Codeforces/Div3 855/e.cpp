...
void dfs(int src, int k, string &p, map<char, set<char>> &list, map<char, set<int>> &freq, set<int> &visited, ll &cntr)
{
    visited.insert(src);
    cntr++;
    for (auto x : list[p[src]])
    {
        for (auto y : freq[x])
            if (!visited.count(y) && (abs(src - y) == k || abs(src - y) == k + 1))
            {
                dfs(y, k, p, list, freq, visited, cntr);
            }
    }
}
