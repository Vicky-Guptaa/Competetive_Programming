...
void solve()
{
    ll n;
    cin >> n;
    map<vector<ll>, int> freq;
    fl(i, 0, n)
    {
        ll freqoe = 0, isPresent = 0;
        string s;
        cin >> s;
        fl(j, 0, s.size())
        {
        ...
        if (__builtin_popcountll(isPresent) != 26)
            freq[{isPresent, freqoe}]++;
    }
    ll cntr = 0;
    ...
    fl(i, 0, info.size())
    {
        fl(j, i + 1, info.size())
        {
            ll p = (info[i][0] | info[j][0]);
            ll q = (info[i][1] ^ info[j][1]);
            if (__builtin_popcountll(p) == 25 && p == q)
            {
                cntr += freq[info[i]] * freq[info[j]];
            }
        }
    }
    cout << cntr << "\n";
