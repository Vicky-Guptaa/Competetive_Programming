...
int Edit(int it1, int it2, string s, string p, vector<vector<int>> &dp, bool isLeft)
{
    int ssize = s.size(), psize = p.size();
    if (it2 == psize)
        return dp[it1][it2] = ssize - it1;

    if (it1 == ssize)
        return dp[it1][it2] = 1e9;

    if (dp[it1][it2] != -1)
        return dp[it1][it2];

    if (ssize - it1 == psize - it2)
    {
        if (s.substr(it1) == p.substr(it2))
            return dp[it1][it2] = 0;
        else
            return dp[it1][it2] = 1e9;
    }
    ...
    if (s[it1] == p[it2])
    {
        return dp[it1][it2] = min(Edit(it1 + 1, it2 + 1, s, p, dp, isLeft) + 1, Edit(it1 + 1, it2, s, p, dp, isLeft) + 1 + isLeft);
    }
    else
    {
        return dp[it1][it2] = Edit(it1 + 1, it2, s, p, dp, isLeft) + 1 + isLeft;
    }
}
...
void solve()
{
    ...
    string s, p;
    cin >> s >> p;
    vector<vector<int>> dpLeft(n + 1, vi(m + 1, -1)), dpRight(n + 1, vi(m + 1, -1));
    ll a1 = Edit(0, 0, s, p, dpLeft, true);
    reverse(vr(s));
    reverse(vr(p));
    ll a2 = Edit(0, 0, s, p, dpRight, false);
    ll ans = min(a1 + 1, a2) >= 1e9 ? -1 : min(a1 + 1, a2);
    // for (auto x : dpLeft)
    // {
    //     cout << x << "\n";
    // }
