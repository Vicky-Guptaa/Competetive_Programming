...
set<ll> helper(ll src, vector<vll> &list, ll n, ll k)
{
    set<ll> res;
    ll timer = ceil(sqrtl(n));
    res.insert(src);
    queue<pll> que;
    que.push({src, 2 * timer});
    vll visited(n + 10, false);
    visited[src] = true;
    while (!que.empty())
    {
        ll curr = que.front().first;
...
void solve()
{
    ...
    ll src = *max_element(vr(inorder));
    fl(i, 0, n)
    {
        if (i == src)
        {
            set<ll> ans = helper(i, list, n, k);
            if (ans.size())
            {
                cout << ans.size() << "\n";
                for (auto x : ans)
