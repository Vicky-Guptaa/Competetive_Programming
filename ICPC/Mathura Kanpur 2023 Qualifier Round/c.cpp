...
int helper(int first, int last, int iter, string &srr, string &brr, vector<vector<int>> &dp)
{
    if (first > last)
    {
        return true;
    ...
    if (srr[iter] == brr[first] && srr[iter] == brr[last])
    {
        return dp[first][last] = (helper(first + 1, last, iter + 1, srr, brr, dp) | helper(first, last - 1, iter + 1, srr, brr, dp));
    }
    else if (srr[iter] == brr[first])
    {
        return dp[first][last] = helper(first + 1, last, iter + 1, srr, brr, dp);
    }
    else if (srr[iter] == brr[last])
    {
        return dp[first][last] = helper(first, last - 1, iter + 1, srr, brr, dp);
    }
    else
    {
        return dp[first][last] = false;
    }
}
...
void solve()
{
    ...
    ll it1 = 0, it2 = n - 1, it = 0;
    vector<vector<int>> dp(n + 1, vector<int>(n + 1, -1));
    if (helper(0, n - 1, 0, srr, brr, dp))
        py else pn
}
/*
