...
void solve()
{
    ...
    lld dx, dy, n;
    cin >> dx >> dy >> n;
    vector<vector<lld>> arr(n, vector<lld>(n + 1, 3));
    ll dist = 1, ans = 0;
    while (dx - dist >= ux && dx + dist <= lx && dy + dist <= uy && dy - dist >= ly)
    {
        bool isFound = true;
        ll add = 1;
        fl(i, 0, n)
        {
            ll ndist = (dx - arr[i][0]) * (dx - arr[i][0]) + (dy - arr[i][1]) * (dy - arr[i][1]);
            if (ndist > dist * dist + arr[i][2] * arr[i][2])
            {
                // add = max(add, (ll)ceil(sqrtl(ndist - dist * dist + arr[i][2] * arr[i][2])));
                isFound = false;
            }
        }
        if (isFound)
