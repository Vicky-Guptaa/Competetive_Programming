...
int helper(int indx, int f, int s, vector<int> &arr, vector<vector<vi>> &dp)
{
    if (indx + 2 == arr.size())
        return 0;
    ...
    ll sum = arr[indx] + 0ll + arr[indx + 1] + 0ll + arr[indx + 2];

    sum += (f > 0) + (s > 0);
    if (sum % 3 == 0)
    {
        return dp[indx][f][s] = helper(indx + 1, f != 0 ? f - 1 : 0, s != 0 ? s - 1 : 0, arr, dp);
    }
    else
    {
        return dp[indx][f][s] = 1e9;
    }
}
...
void solve()
{
    ...
    vi arr(n);
    cin >> arr;
    vector<vector<vector<int>>> dp(n, vector<vi>(3, vi(3, -1)));
    cout << helper(0, 0, 0, arr, dp) << "\n";
}
/*
