...
bool helper(int i, int val, vi &arr, vector<vi> &dp, string &lrb)
{
    if (i == arr.size())
    {
        if (val == 0)
    ...
    lrb += "+";
    dp[i][val] = helper(i + 1, val + arr[i], arr, dp, lrb);
    if (dp[i][val])
        return true;
    lrb.pop_back();
...
void solve()
{
    ...
    arr.push_back(cnt);
    string lrb;
    vector<vi> dp(n, vi(n + 2, -1));
    helper(0, 0, arr, dp, lrb);
    if (!dp[0][0])
    {
        pn
    }
