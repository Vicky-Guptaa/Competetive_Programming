...
void solve()
{
    ...
    ll result = grid[0].back();
    ll prev = result;
    for (int i = 1; i < n; i++)
    {
        if (abs(prev - grid[i][0]) > abs(grid[i].back() - prev)))
            {
                prev = grid[i].front();
                result += abs(grid[i].back() - prev) + abs(grid[i].back() - grid[i].front());
            }
        else
        {
            prev = grid[i].back();
            result += abs(grid[i].front() - prev) + abs(grid[i].back() - grid[i].front());
        }
        // cout << (grid[i].back() - grid[i].front()) << "\n";
    }
    cout << result;
}
/*
