...
void solve()
{
    ...
    if (n >= 6 || n == 4)
    {
        lld strt = ((lld)arr[0] + arr[1]) / 2.0;
        lld end = ((lld)arr[n - 1] + arr[n - 2]) / 2.0;
        lld dist = end - strt;
        cout << dist << "\n";
    }
    else if (n == 5)
    {
        lld strt = ((lld)arr[0] + arr[1]) / 2.0;
        lld end = ((lld)arr[n - 1] + arr[n - 3]) / 2.0;
        lld dist = end - strt;
        strt = ((lld)arr[0] + arr[2]) / 2.0;
        end = ((lld)arr[n - 1] + arr[n - 2]) / 2.0;
        dist = max(dist, end - strt);
        cout << dist << "\n";
    }
}
/*
