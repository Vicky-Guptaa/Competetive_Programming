...
int helper(int indx, vector<int> &fact, int sum, int prod, int n, vector<int> &arr)
{
    if (prod == n)
    {
        for (int i = sum + 1; i <= 41; i++)
            arr.push_back(1);

        if (ans.empty() || ans.size() > arr.size())
            ans = arr;
        // cout << "\n";
        for (int i = sum + 1; i <= 41; i++)
            arr.pop_back();
    ...
    int pos = helper(indx + 1, fact, sum, prod, n, arr);

    if (sum + fact[indx] <= 41 && prod * fact[indx] <= n)
    {
        arr.push_back(fact[indx]);
        pos |= helper(indx, fact, sum + fact[indx], prod * fact[indx], n, arr);
        arr.pop_back();
    }
    return false;
}
