...
void dfs(int x, int y, vector<vector<bool>> &visited, vector<string> &srr)
{
    ...
    fl(i, 0, 4)
    {
        ...
        if (_y < 0 || _x < 0 || _y >= n || _x >= m || srr[_x][_y] == '#' || visited[_x][_y])
            continue;
        dfs(_x, _y, visited, srr);
    }
}
