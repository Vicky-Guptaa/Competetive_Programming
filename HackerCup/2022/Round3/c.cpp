...
void solve()
{
    ...
    for (auto &x : arr)
    {
        if (imap.count(x[1]))
        {
            if (x[3] < *imap[x[1]].rbegin())
            {
                if (prevCost.count(x[0]))
                {
                    fl(i, 0, prevCost[x[0]].size())
                        prevCost[x[1]]
                            .push_back(prevCost[x[0]][i]);
                    fl(i, 0, prevCost[x[1]].size())
                    {
                        prevCost[x[1]][i] += *imap[x[1]].rbegin() - x[3];
                        pque.push(prevCost[x[1]][i]);
                        if (pque.size() > k)
                        {
                            pque.pop();
                        }
                ...
                {
                    prevCost[x[1]].push_back(*imap[x[1]].rbegin() - x[3]);
                    for
                    pque.push(*imap[x[1]].rbegin() - x[3]);
                    if (pque.size() > k)
                    {
                        pque.pop();
                    }
