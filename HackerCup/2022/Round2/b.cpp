...
void solve()
{
    ...
    pll first = {0, 0}, second = {0, 0};
    fl(i, 0, m)
    {
        first.first += brr[i].first * brr[i].first;
        first.first %= mod;
        first.second += brr[i].first;
        first.second %= mod;

        second.first += brr[i].second * brr[i].second;
        second.first %= mod;
        second.second += brr[i].second;
        second.second %= mod;
    }
    ll result = 0;
    fl(i, 0, n)
    {
        // first -> X
        ll calc = 0;
        ll sqre = (m * ((arr[i].first * arr[i].first) % mod)) % mod;

        calc = (sqre + first.first) % mod;

        calc -= 2 * first.second * arr[i].first;

        calc = ((calc) % mod + mod) % mod;
        ...
        // second -> Y
        calc = 0;
        sqre = (m * ((arr[i].second * arr[i].second) % mod)) % mod;
        calc = (sqre + second.first) % mod;

        calc -= 2 * second.second * arr[i].second;
