...
ll helper(int strt, int ki, int zi, bool isLeft, int k, int z, vll &arr, vector<vector<vector<int>>> &dp)
{
    if (ki == k)
        return 0;

    if (dp[ki][zi][isLeft] != -1)
        return dp[ki][zi][isLeft];

    if (!isLeft && z > zi && strt > 0)
    {
        dp[ki][zi][isLeft] = max(arr[strt - 1] + helper(strt - 1, ki + 1, zi + 1, 1, k, z, arr, dp),
                                 arr[strt + 1] + helper(strt + 1, ki + 1, zi, 0, k, z, arr, dp));
    }
    else
    {
        dp[ki][zi][isLeft] = arr[strt + 1] + helper(strt + 1, ki + 1, zi, 0, k, z, arr, dp);
    }
    return dp[ki][zi][isLeft];
}
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    ll ans = helper(0, 0, 0, 1, k, z, arr, dp) + arr[0];
    cout << ans << '\n';
}
/*
