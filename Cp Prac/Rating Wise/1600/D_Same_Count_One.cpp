...
void solve()
{
    ...
    fl(i, 0, n)
    {
        ...
        freq[accumulate(vr(mat[i]), 0ll)].insert(i);
        sum += accumulate(vr(mat[i]), 0ll);
    }
    if (sum % n != 0)
    {
        pm return;
    ...
    while (!freq.empty())
    {
        ...
        int low = *freq.begin()->second.begin();
        int lc = freq.begin()->first;
        int high = *freq.rbegin()->second.begin();
        int hc = freq.rbegin()->first;
        vi remove;
        for (auto x : oneIndx[high])
        {
            if (!mat[low][x])
            {
                lc++;
                hc--;
                remove.push_back(x);
                res.push_back({high + 1, low + 1, x + 1});
                // cout << high + 1 << " " << low + 1 << " " << x + 1 << "\n";
                mat[high][x] = 0;
                mat[low][x] = 1;
                if (lc == target || hc == target)
                    break;
            }
        }
        fl(i, 0, remove.size()) oneIndx[high].erase(remove[i]);
        if (lc != target)
        {
            freq[lc].insert(low);
        }
        freq.begin()->second.erase(low);
        ...
        freq.rbegin()->second.erase(high);
        if (freq.rbegin()->second.empty())
            freq.erase(freq.rbegin()->first);
    }
    cout << res.size() << "\n";
    for (auto x : res)
    {
