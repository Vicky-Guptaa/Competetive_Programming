...
int helper(int dig, int oper, vector<vector<int>> &dp)
{
    if (oper == 0)
        return 1;
    ...
    if (dig == 9)
    {
        return dp[dig][oper] = (helper(1, oper - 1, dp) + helper(0, oper - 1, dp)) % mod;
    }
    else
    {
        return dp[dig][oper] = helper(dig + 1, oper - 1, dp) % mod;
    }
}
...
int main()
{
    ...
    You Can Do_It
        ll t;
    cin >> t;
    vector<vector<int>> dp(10, vector<int>(2e5 + 1, -1));
    fl(i, 0, t)
    {
        solve(dp);
    }
