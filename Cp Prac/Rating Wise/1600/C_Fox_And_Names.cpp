...
vector<int> detectCycle(vector<int> list[])
{
    vector<int> in(26, 0);
    fl(i, 0, 26)
    {
...
void solve()
{
    ...
    // fl(i, 0, n)
    // {
    //     if (mpIndx.count(srr[i][0]))
    //     {
    //         if (i - mpIndx[srr[i][0]] > 1)
    //         {
    //             cout << "Impossible\n";
    //             return;
    //         }
    ...
    fl(i, 0, n)
    {
        fl(j, i + 1, n)
        {
            int size = min(srr[i].size(), srr[j].size());
            int diff = size;
            fl(k, 0, size)
            {
                if (srr[i][k] != srr[j][k])
                {
                    diff = k;
                    list[srr[i][k] - 'a'].push_back(srr[j][k] - 'a');
                    break;
                }
            }
            if (diff == size && srr[i].size() > srr[j].size())
            {
                cout << "Impossible\n";
                return;
            }
    ...
    vector<int> topo = detectCycle(list);
    if (topo.empty())
    {
        cout << "Impossible\n";
    }
