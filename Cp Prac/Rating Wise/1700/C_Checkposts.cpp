...
void dfs(int src, vector<int> &visit, vector<int> list[], vector<int> &st)
{
    visit[src] = true;
    for (auto child : list[src])
    {
        if (visit[child])
            continue;
        dfs(child, visit, list, st);
    }
    st.push_back(src);
}
void dfs1(int src, vector<int> &visit, vector<int> list[], map<int, int> costs[], int i, vll &arr)
{
    visit[src] = true;
    for (auto child : list[src])
    {
        if (visit[child])
            continue;
        dfs1(child, visit, list, costs, i, arr);
    }
    costs[i][arr[src - 1]]++;
}
// Code
void solve()
{
    ...
    for (auto &x : visit)
        x = 0;
    map<int, int> costs[n + 1];
    reverse(vr(st));
    for (int i = 1; i <= n; i++)
    {
        if (visit[st[i - 1]])
            continue;
        dfs1(st[i - 1], visit, rlist, costs, i, arr);
    }
    ll result = 1;
    ll cost = 0;
    fl(i, 0, n + 1)
    {
        if (costs[i].empty())
            continue;
        result = product(result, costs[i].begin()->second);
        cost += costs[i].begin()->first;
    }
    cout << cost << " " << result << "\n";
}
/*
