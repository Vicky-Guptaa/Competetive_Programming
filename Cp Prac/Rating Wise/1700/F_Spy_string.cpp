...
int helper(int indx, int isV, ll n, ll m, string &curr, vector<string> &srr, vector<vi> &dp)
{
    if (indx == m)
    {
        // cout << curr << "\n";
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        curr.push_back(ch);
        if (isPossible && helper(indx + 1, newV, n, m, curr, srr, dp))
        {
            return dp[indx][isV] = true;
        }
        curr.pop_back();
    }
    return dp[indx][isV] = false;
}
