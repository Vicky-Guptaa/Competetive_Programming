...
bool isPossible(map<int, int> freq, int curr, vpll &prr)
{
    while (!freq.empty())
    {
        ll first = freq.rbegin()->first;
        freq[first]--;
        if (freq[first] == 0)
            freq.erase(first);
        if (freq.count(curr - first))
        {
            freq[curr - first]--;
            if (freq[curr - first] == 0)
        ...
        prr.push_back({curr - first, first});
        curr = first;
    }
    return true;
}
...
void solve()
{
    ...
    ll curr = freq.rbegin()->first;

    freq.rbegin()->second--;
    if (freq.rbegin()->second == 0)
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        if (freq[arr[i]] == 0)
            freq.erase(arr[i]);
        prr.push_back({arr[i], curr});
        if (isPossible(freq, curr, prr))
        {
            py cout << curr + arr[i] << "\n";
            for (auto x : prr)
            {
