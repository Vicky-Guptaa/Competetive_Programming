...
int helper(int indx, int time, ll n, ll h, ll l, ll r, vll &arr, vector<vi> &dp)
{
    if (indx == n)
    {
    ...
    if (indx == 0)
    {
        return dp[indx][time] = max(helper(indx + 1, (time + arr[indx]) % h, n, h, l, r, arr, dp),
                                    helper(indx + 1, (time + arr[indx] - 1) % h, n, h, l, r, arr, dp));
    }
    else
    {
        bool isInRange = false;
        if (l <= time && r >= time)
            isInRange = true;
        return dp[indx][time] = isInRange + max(helper(indx + 1, (time + arr[indx]) % h, n, h, l, r, arr, dp),
                                                helper(indx + 1, (time + arr[indx] - 1) % h, n, h, l, r, arr, dp));
    }
}
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    vector<vector<int>> dp(n, vi(h + 1, -1));
    cout << helper(0, 0, n, h, l, r, arr, dp) << "\n";
}
/*
