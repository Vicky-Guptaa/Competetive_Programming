...
int binSearch(vpll &arr, ll val)
{
    int low = 0, high = (int)arr.size() - 1, answer = 0;
    while (low <= high)
    {
...
void solve()
{
    ...
    cin >> arr >> brr;
    ll res = 1e9;
    vpll t1 = {{arr[0], 0}}, t2 = {{brr[0], 0}};

    fl(i, 1, n)
    {
        if (t1.back().first > arr[i])
        {
            t1.push_back({arr[i], i});
        }
    }
    fl(i, 1, n)
    {
        if (t2.back().first < brr[i])
        {
            t2.push_back({brr[i], i});
        }
    }
    
    for (int i = 0; i < t1.size(); i++)
    {
        int indx = binSearch(t2, t1[i].first);
        res = min(res, t1[i].second + indx);
    }
    cout << res << "\n";
}
/*
