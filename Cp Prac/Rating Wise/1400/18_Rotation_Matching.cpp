...
void rotate_arry(vll &brr, int rot)
{
    reverse(vr(brr));
    reverse(brr.begin(), brr.begin() + (int)brr.size() - rot);
    reverse(brr.begin() + (int)brr.size() - rot, brr.end());
}
...
void solve()
{
    ...
    cin >> arr >> brr;
    mll maped, freq;
    fl(i, 0, n) maped[arr[i]] = i;

    fl(i, 0, n)
    {
        int rot = i - maped[brr[i]];
        if (rot < 0)
            rot += n;
