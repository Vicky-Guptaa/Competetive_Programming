...
void solve()
{
    ...
    fl(i, 0, n) freq[arr[i]].insert(brr[i]);
    ll res_time = freq.rbegin()->first;
    ll self_time = 0;
    while (!freq.empty())
    {
        if (freq.rbegin()->first > *freq.rbegin()->second.begin())
        {
            if (self_time + *freq.rbegin()->second.begin() >= res_time)
                break;

            self_time += *freq.rbegin()->second.begin();
            freq.rbegin()->second.erase(freq.rbegin()->second.begin());
            if (freq.rbegin()->second.empty())
                freq.erase(freq.rbegin()->first);

            if (!freq.empty())
                res_time = freq.rbegin()->first;
            else
                res_time = 0;
        }
        else
