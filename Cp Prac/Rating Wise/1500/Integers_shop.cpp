...
void solve()
{
    ...
    cin >> arr;
    ll mLeft = 0, mRight = 0, mAll = 0;
    cout << arr[0][2] << "\n";
    fl(i, 1, n)
    {
        if ((arr[i][0] < arr[mLeft][0]) || (arr[i][0] == arr[mLeft][0] && arr[i][2] < arr[mLeft][2]))
        {
            mLeft = i;
        }
        if ((arr[i][1] > arr[mRight][1]) || (arr[i][1] == arr[mRight][1] && arr[i][2] < arr[mRight][2]))
        {
            mRight = i;
        }
        if ((arr[i][0] <= arr[mAll][0] && arr[i][1] > arr[mAll][1]) || (arr[i][0] < arr[mAll][0] && arr[i][1] >= arr[mAll][1]) ||
            (arr[i][0] == arr[mAll][0] && arr[i][1] == arr[mAll][1] && arr[i][2] < arr[mAll][2]))
        {
            mAll = i;
        }
        if (arr[mAll][0] == arr[mLeft][0] && arr[mAll][1] == arr[mRight][1] && arr[mAll][2] <= arr[mLeft][2] + arr[mRight][2])
        {
            cout << arr[mAll][2] << "\n";
        }
        else
        {
            cout << arr[mLeft][2] + arr[mRight][2] << "\n";
        }
    }
}
/*
