...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    ll minO = arr[1], minE = arr[0];
    ll res = arr[0] * n + arr[1] * n;
    ll sumO = arr[1], sumE = arr[0];
    fl(i, 2, n)
    {
        if (i & 1)
            minO = min(arr[i], minO), sumO += arr[i];
        else
            minE = min(arr[i], minE), sumE += arr[i];
        res = min(res, sumE + sumO + minO * (n - (i - 1) / 2) + minE * (n - i / 2) - minO - minE);
    }
    cout << res << "\n";
}
/*
