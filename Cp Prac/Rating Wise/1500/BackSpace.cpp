...
int helper(int s, string &p, int back, map<char, set<int>> &oset, map<string, int> &dp)
{
    if (s == p.size())
        return true;
    ...
    for (auto x : oset[p[s]])
    {
        if (back == -1)
        {
            if (helper(s + 1, p, x, oset, dp))
            {
                return dp[state] = true;
            }
        }
        else if (x - back > 0 && (x - back) % 2 == 1)
        {
            if (helper(s + 1, p, x, oset, dp))
            {
                return dp[state] = true;
...
void solve()
{
    ...
    int strt = (slen - plen) & 1;
    int indx = 0;
    for (int i = strt; i < slen; i++)
    {
        if (s[i] != p[indx])
        {
            i++;
