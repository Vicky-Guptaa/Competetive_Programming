...
void solve()
{
    ...
    fl(i, 0, 10)
    {
        mxCntr = max(mxCntr, freq[i]);
    }
    map<string, int> ffreq;
    fl(j, 0, 100)
    {
        int mode = 0;
        string toFind = to_string(j);
        if (j <= 9)
            toFind = "0" + toFind;
        if (toFind[0] == toFind[1])
            continue;
        int cntr = 0;
        fl(i, 0, n)
        {
            if (s[i] == toFind[0] && (mode == 0))
            {
                mode = 1;
            }
            else if (s[i] == toFind[1] && (mode == 1))
            {
                mode = 0;
                cntr++;
            }
        }
        mxCntr = max(mxCntr, cntr * 2);
    }
    cout << n - mxCntr << "\n";
}
/*
