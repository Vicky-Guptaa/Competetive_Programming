...
void solve()
{
    ...
    fl(i, 0, k)
    {
        room[iindx[i] - 1] = condi[i];
        pque.push({condi[i], iindx[i] - 1});
    }
    while (!pque.empty())
    {
        auto node = pque.top();
        ...
        if (node.second > 0 && room[node.second - 1] > node.first + 1)
        {
            room[node.second - 1] = node.first + 1;
            pque.push({node.first + 1, node.second - 1});
        }
        if (node.second < n - 1 && room[node.second + 1] > node.first + 1)
        {
            room[node.second + 1] = node.first + 1;
            pque.push({node.first + 1, node.second + 1});
        }
