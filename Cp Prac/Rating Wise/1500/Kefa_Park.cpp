...
void dfs(int src, int m, int ref, vector<int> list[], vll &arr, vi &visited, int &ans)
{
    if (m < ref)
        return;
    ...
    for (auto curr : list[src])
    {
        ...
        if (arr[curr])
        {
            dfs(curr, m, ref + 1, list, arr, visited, ans);
        }
        else
        {
            dfs(curr, m, 0, list, arr, visited, ans);
        }
    }
    if (cntr == 0)
    {
...
void solve()
{
    ...
    dfs(1, m, ref, list, arr, visited, ans);
    cout << ans << '\n';
}
/*
