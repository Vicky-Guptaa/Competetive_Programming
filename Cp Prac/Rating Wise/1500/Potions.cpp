...
ll helper(int s, int i, vll &arr, vector<vll> &dp)
{
    if (s == arr.size())
        return -1;
    if (dp[s][i] != -1)
        return dp[s][i];
    dp[s][i] = max(helper(s + 1, i, arr, dp), helper(s + 1, i + 1, arr, dp) + arr[s]);
    return dp[s][i];
}
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= n; j++)
        {
            if (dp[j - 1][i - 1] + arr[j - 1] >= 0 || dp[j - 1][i] >= 0)
                dp[j][i] = max(dp[j - 1][i - 1] + arr[j - 1], dp[j - 1][i]);
            if (dp[j][i] >= 0)
                ans = max(ans, i);
        }
    }
