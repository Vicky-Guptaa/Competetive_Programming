...
int helper(int src, int par, vll list[], vi &ans, map<pair<int, int>, int> &mp)
{
    int cnt = 1;
    for (auto x : list[src])
    {
        if (par != x)
        {
            cnt += helper(x, src, list, ans, mp);
        }
    }
    if (cnt == 3)
    {
        if (mp.count({min(src, par), max(src, par)}))
        {
            ans.push_back(mp[{min(src, par), max(src, par)}]);
        }
        return cnt - 3;
    }
    return cnt;
