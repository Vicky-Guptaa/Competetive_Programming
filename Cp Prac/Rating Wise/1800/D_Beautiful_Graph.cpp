...
bool isBipartite(ll src, int c, vi &visit, vi list[], vi &color)
{
    color[src] = c;
    visit[src] = true;
    for (auto child : list[src])
    {
        ...
        {
            if (!isBipartite(child, c == 1 ? 2 : 1, visit, list, color))
                return false;
        }
    }
    return true;
...
void helper(int src, vector<int> &visit, vi list[], ll &acntr, ll &bcntr, vi &color)
{
    if (color[src] == 1)
        acntr++;
    else
    ...
    for (auto child : list[src])
    {
        if (visit[child])
            continue;
        helper(child, visit, list, acntr, bcntr, color);
    }
}
...
void solve()
{
    ...
    fl(i, 1, n + 1)
    {
        if (visit[i])
            continue;
        if (!isBipartite(i, 1, visit, list, color))
        {
            cout << "0\n";
            return;
        }
    ...
    fl(i, 1, n + 1)
    {
        ...
        ll acntr = 0, bcntr=0;
        // cout<<cntr<<"\n";
        helper(i, visit, list, acntr, bcntr, color);
        counter = product(counter, sum(power(2, acntr, mod), power(2, bcntr, mod),mod), mod);
        counter = counter % mod;
    }
    cout << counter << "\n";
}
