...
bool mycomp(ppi &a, ppi &b)
{
    return a.first < b.first;
}

ll helper(int left, int right, vpi &arr, vector<vll> &dp)
{
    if (left == -1 && right == arr.size())
    {
        return 0;
    }
    ...
    if (left == -1)
    {
        return dp[left + 1][right] = helper(left, right + 1, arr, dp) + abs(arr[right].ff - arr[left + 1].ff + 0ll) * 1ll * arr[right].ss;
    }
    else if (right == arr.size())
    {
        return dp[left + 1][right] = helper(left - 1, right, arr, dp) + abs(arr[right - 1].ff - arr[left].ff + 0ll) * 1ll * arr[left].ss;
    }
    else
    {
        return dp[left + 1][right] = min(helper(left - 1, right, arr, dp) + abs(arr[right - 1].ff - arr[left].ff + 0ll) * 1ll * arr[left].ss,
                                         helper(left, right + 1, arr, dp) + abs(arr[right].ff - arr[left + 1].ff + 0ll) * 1ll * arr[right].ss);
    }
}
...
void solve()
{
    ...
    for (auto x : freq)
    {
        brr.push_back({x.ff, x.ss});
    }
    sort(vr(brr), mycomp);

    vector<vector<ll>> dp(n + 2, vll(n + 2, -1));
    ll ans = 1e18;
    fl(i, 1, brr.size())
    {
        ans = min(helper(i - 1, i, brr, dp), ans);
    }
    cout << ans << '\n';
}
/*
