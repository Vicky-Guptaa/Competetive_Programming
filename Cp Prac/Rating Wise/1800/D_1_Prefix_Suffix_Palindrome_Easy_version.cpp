...
void solve()
{
    ...
    n = ns.size();
    ll mxLen1 = 0, mxLen2 = n;
    fl(i, 0, n)
    {
        if (isPal(ns, 0, i))
            mxLen1 = max(mxLen1, (ll)i);
    }
    string result1 = s.substr(0, left) + ns.substr(0, mxLen1 + 1) + s.substr(right + 1, (int)s.size() - right);
    rl(i, n - 1, 0)
    {
        if (isPal(ns, i, n - 1))
            mxLen2 = min(mxLen2, (ll)i);
    }
    string result2 = s.substr(0, left) + ns.substr(mxLen2, n - mxLen2) + s.substr(right + 1, (int)s.size() - right);
