...
ll helper(int strt, int end, int val, vll &arr)
{
    if (strt == end)
    {
        return 1;
    ...
    ll mn = *min_element(arr.begin() + strt, arr.begin() + end + 1);
    ll cost = mn - val, prev = strt;

    for (int i = strt; i <= end; i++)
    {
        if (arr[i] == mn)
        {
            cost += helper(prev, i - 1, mn, arr);
            prev = i + 1;
        }
    }
    if (prev != end + 1)
    {
        cost += helper(prev, end, mn, arr);
    }
    cost = min(cost, end - strt + 1ll);
    return cost;
}
