...
void dikjstra(vpll list[], vll &dist)
{
    ...
    while (!pque.empty())
    {
        auto par = pque.top();
        pque.pop();
        for (auto child : list[par.second])
        {
            if (dist[child.first] > par.first + child.second)
            {
                dist[child.first] = par.first + child.second;
                pque.push({par.first + child.second, child.first});
            }
        }
    }
    while (!pque.empty())
