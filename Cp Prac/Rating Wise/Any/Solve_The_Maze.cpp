...
bool bfs(vector<vll> &visit, vector<string> &srr, queue<vll> &que, ll &iter)
{
    ll n = srr.size(), m = srr[0].size();
    vll moveX = {0, 0, -1, 1};
    vll moveY = {-1, 1, 0, 0};
    while (!que.empty())
    {
        ...
        fl(i, 0, 4)
        {
            ...
            if (_x >= n || _y >= m || _x < 0 || _y < 0 || srr[_x][_y] == 'B' || srr[_x][_y] == '#')
                continue;
            if (visit[_x][_y] == iter)
                continue;
            if (visit[_x][_y])
...
void solve()
{
    ...
    vector<vll> visit(n, vll(m, false));
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (srr[i][j] == 'B')
            {
                fl(p, 0, 4)
                {
                    int x = i + moveX[p], y = j + moveY[p];
                    if (x >= n || y >= m || x < 0 || y < 0 || srr[x][y] == '#' || srr[x][y] == 'B')
                        continue;
                    if (srr[x][y] == 'G')
                    {
                        pn return;
