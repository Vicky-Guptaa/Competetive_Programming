...
void solve()
{
    ...
    int cntr = 0, n = nums.size(), itr = n / 2;
    for (int i = 0; i < n / 2; i++)
    {
    ...
    while (!freq.empty())
    {
        auto curr = freq.lower_bound(freq.rbegin()->first / 2);
        if (curr == freq.begin() && curr->first * 2 > freq.rbegin()->first)
        {
            freq.erase(freq.rbegin()->first);
            continue;
        }
        if (curr->first * 2 > freq.rbegin()->first)
            curr--;
        freq[curr->first]--;
        freq[freq.rbegin()->first]--;
        if (freq[curr->first] == 0)
            freq.erase(curr->first);
        if (freq[freq.rbegin()->first] == 0)
            freq.erase(freq.rbegin()->first);
        cntr += 2;
    }
    cout << cntr / 2 + (n - cntr) << "\n";
