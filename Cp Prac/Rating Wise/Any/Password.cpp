...
public:
    ...
    {
        // 1 based indexing
        int hres = diff(hash[r], hash[l - 1], mod);
...
void solve()
{
    ...
    rl(i, n - 1, 0)
    {
        if (shash.getHash(1, i + 1) == shash.getHash(n - i, n))
        {
            res.push_back(i + 1);
        }
    }
    ...
    reverse(vr(res));
    string word;
    int low = 0, high = (int)res.size() - 1, ans = -1;
    while (low <= high)
    {
        int mid = (low + high) / 2;
        word = s.substr(0, res[mid]);
        int indx = s.find(word, 1);
        if (indx != string::npos && indx != n - res[mid])
        {
            ans = mid;
            low = mid + 1;
        }
