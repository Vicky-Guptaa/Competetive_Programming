...
void solve()
{
    ...
    if (isType1)
    {
        ll num = stol(s.substr(indx));
        cout << "R" << num;
        string first = s.substr(0, indx);
        num = 0;
        fl(i, 0, first.size())
        {
            num += (first[i] - 'A' + 1) * powl(26ll, (int)first.size() - i - 1);
        }
        cout << "C" << num << "\n";
    }
    else
    {
        ll cindx = s.find("C");
        ll num1 = stoi(s.substr(indx, cindx - indx));
        ll num2 = stoi(s.substr(cindx + 1));
        ll temp = num2;
        string first;
        while (num2)
        {
            first += (num2 % 26 == 0) ? char('Z') : char('A' + diff(num2 % 26, 1, 26));
            num2 /= 26;
            num2 -= first.back() == 'Z' ? 1 : 0;
        }
        reverse(vr(first));
        cout << first << num1 << "\n";
    }
