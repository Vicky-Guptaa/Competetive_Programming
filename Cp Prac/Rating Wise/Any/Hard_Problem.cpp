...
bool isCompare(int strt, bool isRev, vector<string> &srr, bool curr)
{
    string prevs = srr[strt - 1];
    if (isRev)
        reverse(vr(prevs));
    string currs = srr[strt];
    if (curr)
        reverse(vr(currs));
    return currs >= prevs;
}

ll helper(int strt, bool isRev, vector<string> &srr, vll &arr, vector<vll> &dp)
{
    if (strt == arr.size())
    {
        return 0;
    ...
    if (dp[strt][isRev] != -1)
        return dp[strt][isRev];
    ll cost = 1e18;
    if (isCompare(strt, isRev, srr, 0))
    {
        cost = min(cost, helper(strt + 1, 0, srr, arr, dp));
    }

    if (isCompare(strt, isRev, srr, 1))
    {
        cost = min(cost, helper(strt + 1, 1, srr, arr, dp) + arr[strt]);
    }

    return dp[strt][isRev] = cost;
}
...
void solve()
{
    ...
    vector<string> srr(n);
    cin >> srr;
    vector<vector<long long>> dp(n, vll(2, -1));
    ll h1 = helper(1, 0, srr, arr, dp);
    ll h2 = helper(1, 1, srr, arr, dp) + arr[0];
    if (min(h1, h2) == 1e18)
    {
        pm
    }
