...
bool helper(int iter, bool ristrict, string &num, ll &k, string n, string &ans)
{
    ...
    int start = (ristrict) ? num[iter] - '0' : 0;
    if (ristrict)
    {
        if (iter == 0)
        {
            if (helper(iter + 1, true, num, k, n, ans))
                return true;
        }
        else
        {
            if (helper(iter + 1, true, num, k, n + to_string(start), ans))
                return true;
        }
        start++;
    }
    fl(i, start, 10)
    {
        if (helper(iter + 1, false, num, k, n + to_string(i), ans))
            return true;
    }
    return false;
}
...
void solve()
{
    ll n, k;
    cin >> n >> k;
    string num = "0" + to_string(n), ans;
    helper(0, true, num, k, "", ans);
    cout << ans << "\n";
}
/*
