...
void solve()
{
    ...
    ll mn_points = 1e9, points = 0;
    priority_queue<int, vi, greater<int>> pque3;
    while (m > 0 && !pque1.empty())
    {
        m -= pque1.top();
        pque3.push(pque1.top());
        pque1.pop();
        points += 2;
    }
    while (m > 0)
    ...
    while (!pque3.empty())
    {
        ...
        if (m <= 0)
            mn_points = min(mn_points, points);
        else
            break;
    }
    cout << mn_points << "\n";
