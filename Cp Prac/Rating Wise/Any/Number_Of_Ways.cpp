...
{
    ll low = i + 1, high = (int)preSum.size() - 1, ans = -1;
    while (low <= high)
    {
        ll mid = (low + high) / 2;
...
void solve()
{
    ...
    vll preSum(n);
    preSum[0] = arr[0];
    fl(i, 1, n) preSum[i] = preSum[i - 1] + arr[i];
    ll ans = 0;
    fl(i, 0, n)
    {
        ll first = preSum[i];
        ll secondIndx = search(preSum, first, first, i);
        if (secondIndx == -1 || first * 3 != preSum.back())
        {
            continue;
        }
        if (preSum[secondIndx] == first)
        {
            ans++;
        }
    }
