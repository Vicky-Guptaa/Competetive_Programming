...
ll helper(ll itr, ll k, ll n, ll m, vll &preSum, vector<vll> &dp)
{
    if (k == 0)
        return 0;
    if (itr >= n + 1)
    ...
    if (k != 0 && itr + m <= n + 1)
    {
        dp[itr][k] = preSum[itr + m - 1] - preSum[itr - 1] + helper(itr + m, k - 1, n, m, preSum, dp);
    }

    return dp[itr][k] = max(dp[itr][k],
                            helper(itr + 1, k, n, m, preSum, dp));
}
