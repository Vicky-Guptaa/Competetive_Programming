...
ll helper(int strt, bool isDTaken, ll n, ll k, ll d, vector<vll> &dp)
{
    if (strt == n)
        return isDTaken;
    if (strt > n)
        return false;
    if (dp[strt][isDTaken] != -1)
        return dp[strt][isDTaken];
    ll ways = 0;
    for (int i = 1; i <= k; i++)
    {
        ways += helper(strt + i, (isDTaken | (i >= d)), n, k, d, dp);
        ways %= mod;
    }
    return dp[strt][isDTaken] = ways;
}
