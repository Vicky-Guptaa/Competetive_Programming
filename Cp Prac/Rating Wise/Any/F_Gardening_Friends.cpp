...
int dfs(int src, int par, vll list[], vi &hght)
{
    int h = 0;
    for (auto child : list[src])
    {
        if (par != child)
        {
            h = max(h, dfs(child, src, list, hght));
        }
    }
    return hght[src] = h + 1;
}

void dfs2(int src, int par, int c, vll list[], vi &hght, ll &ans, int cc, int l, int nc)
{
    int fmx = 0, smx = 0;
    for (auto child : list[src])
    {
    ...
    for (auto child : list[src])
    {
        if (par != child)
        {
            if (fmx == hght[child])
            {
                dfs2(child, src, c + 1, list, hght, ans, cc, l, max(nc + 1, smx > 0 ? smx + 2 : 1));
            }
            else
            {
                dfs2(child, src, c + 1, list, hght, ans, cc, l, max(nc + 1, fmx > 0 ? fmx + 2 : 1));
            }
        }
    }
    ans = max(ans, (hght[src] - 1) * 1ll * l - cc * 1ll * c);
    ans = max(ans, (nc - 1) * 1ll * l - cc * 1ll * c);
}
...
void solve()
{
    ...
    vector<int> hght(n + 1, 0);
    dfs(1, -1, list, hght);
    ll ans = 0;
    dfs2(1, -1, 0, list, hght, ans, c, l, 0);
    cout << ans << "\n";
}
/*
