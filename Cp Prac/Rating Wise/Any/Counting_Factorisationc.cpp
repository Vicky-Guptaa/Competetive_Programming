...
ll helper(ll itr, ll n, vpll &arr, ll mod, vector<vll> &dp, vi &prime)
{
    if (itr == arr.size())
    {
        return (n == 0) ? fact[dp[0].size() / 2] : 0;
    }
    if (n < 0)
        return 0;
    if (dp[itr][n] != -1)
    ...
    dp[itr][n] = (helper(itr + 1, n - arr[itr].second, arr, mod, dp, prime) * ifact[arr[itr].second]) % mod;
    if (prime[arr[itr].first] == 1)
        dp[itr][n] + = (helper(itr + 1, n - (arr[itr].second - 1), arr, mod, dp, prime) * ifact[arr[itr].second - 1]) %
                       mod;
    return dp[itr][n] %= mod;
}
...
void solve()
{
    ...
    vll arr(2 * n);
    cin >> arr;
    vector<int> prime = Prime_Sieve(1e6);
    set<int> oset;
    mll freq;
    fl(i, 0, 2 * n)
    {
    ...
    for (auto x : freq)
    {
        narr.push_back(make_pair(x.first, x.second));
    }
    vector<vll> dp(narr.size() + 1, vll(arr.size() + 1, -1));
    cout << helper(0, n, narr, val, mod, dp, prime) << "\n";
}
/*
