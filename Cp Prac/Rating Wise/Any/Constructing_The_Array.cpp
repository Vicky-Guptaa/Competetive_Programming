...
void helper(int &iter, vll &arr, priority_queue<vll, vector<vll>, greater<vll>> &pque)
{
}
...
void solve()
{
    ...
    cin >> n;
    vll arr(n + 1, 0);
    priority_queue<vll, vector<vll>, mycomp> pque;
    pque.push({n, 1, n});
    int iter = 1;
    while (!pque.empty())
    {
        auto curr = pque.top();
        pque.pop();
        ll mid = (curr[1] + curr[2]) / 2;
        arr[mid] = iter;
        iter++;
        if (curr[1] == curr[2])
            continue;
        if (mid != curr[1])
            pque.push({mid - curr[1], curr[1], mid - 1});
        if (mid != curr[2])
            pque.push({curr[2] - mid, mid + 1, curr[2]});
    }
