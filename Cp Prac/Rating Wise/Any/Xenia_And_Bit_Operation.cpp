...
*/
    ...
    node Combine(node &left, node &right, int lvl)
    {
        return node((lvl & 1) ? left.val ^ right.val : left.val | right.val, 0);
    }

    void constructSegTree(int index, int startIndx, int endIndx, int lvl = 1)
    {
        ...
        constructSegTree(2 * index + 1, startIndx, midIndx, lvl + 1);
        constructSegTree(2 * index + 2, midIndx + 1, endIndx, lvl + 1);
        segArr[index] = Combine(segArr[2 * index + 1], segArr[2 * index + 2], hght - lvl);
    }
    node getQuery(int index, int startIndx, int endIndx, int lquery, int rquery, int lvl = 1)
    {
        // no overlap
        ...
        // partial overlap
        int midIndx = startIndx + (endIndx - startIndx) / 2;
        node left = getQuery(index * 2 + 1, startIndx, midIndx, lquery, rquery, lvl + 1);
        node right = getQuery(index * 2 + 2, midIndx + 1, endIndx, lquery, rquery, lvl + 1);

        return Combine(left, right, hght - lvl);
    }

    void updateQuery(int index, int startIndx, int endIndx, int position, int val, int lvl = 1)
    {
        ...
        int midIndx = startIndx + (endIndx - startIndx) / 2;
        updateQuery(2 * index + 1, startIndx, midIndx, position, val, lvl + 1);
        updateQuery(2 * index + 2, midIndx + 1, endIndx, position, val, lvl + 1);
        segArr[index] = Combine(segArr[2 * index + 1], segArr[2 * index + 2], hght - lvl);
    }
...
void solve()
{
    ...
    fl(i, 0, m)
    {
        ll p, q;
        cin >> p >> q;
        tree.updateQuery(p - 1, q);
        cout << tree.getQuery(0, (int)arr.size() - 1).val << "\n";
    }
}
/*
