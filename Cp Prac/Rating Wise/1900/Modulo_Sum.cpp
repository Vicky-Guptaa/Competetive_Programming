...
int helper(int strt, ll sum, int m, vpi &arr, vector<vector<int>> &dp)
{
    if (sum != 0 && sum % m == 0)
        return true;
    if (strt == arr.size())
    ...
    if (dp[strt][sum % m] != -1)
        return dp[strt][sum % m];

    for (int i = 0; i <= arr[strt].second; i++)
    {
        ll nsum = (sum + arr[strt].first * i);
        if (helper(strt + 1, nsum, m, arr, dp))
            return dp[strt][sum % m] = true;
    }
    return dp[strt][sum % m] = false;
}
...
void solve()
{
    ...
    vector<vector<int>> dp(brr.size(), vector<int>(m + 1, -1));
    if (helper(0, 0, m, brr, dp) || brr[0].first == 0)
        py else pn
}
/*
