...
void solve()
{
    ...
    fl(i, 0, n - 1)
    {
        if (s1.Find(arr[i].first) == s1.Find(arr[i].second))
        {
            brr.push_back(arr[i]);
            continue;
        }
        s1.Union(arr[i].first, arr[i].second);
    }
    cout << brr.size() << "\n";
    ll iter = 0;
    fl(j, 1, n + 1)
    ...
    fl(i, 2, n + 1)
    {
        if (s1.parent[1] != s1.parent[i])
        {
            cout << brr[iter] << " " << 1 << " " << i << "\n";
            s1.Union(s1.parent[1], s1.parent[i]);
            fl(j, 1, n + 1)
            {
                s1.Find(j);
            }
            oset.insert({s1.parent[1], s1.parent[i]});
            iter++;
        }
    }
}
