...
int isPalindrome(vll &arr, int left, int right)
{
    int len = 0;
...
ll helper(int strt, int end, vll &arr, vector<vll> &dp)
{
    if (strt > end)
        return 0;
    if (strt == end)
    ...
    if (dp[strt][end] != -1)
        return dp[strt][end];
    ll mn = helper(strt + 1, end, arr, dp) + 1;
    if (arr[strt] == arr[strt + 1])
        mn = min(helper(strt + 2, end, arr, dp) + 1, mn);
    fl(i, strt + 2, end + 1)
    {
        if (arr[strt] == arr[i])
            mn = min(helper(strt + 1, i - 1, arr, dp) + helper(i + 1, end, arr, dp), mn);
    }
    return dp[strt][end] = mn;
}
