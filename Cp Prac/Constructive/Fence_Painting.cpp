...
void solve()
{
    ll n, m;
    cin >> n >> m;
    vll arr(n), brr(n), crr(m);
    cin >> arr >> brr >> crr;

    map<int, multiset<int>> omap;
    fl(i, n)
    {
        if (brr[i] == arr[i])
            continue;
    ...
    vector<int> noFound(m, 0);
    for (int i = 0; i < m; i++)
    {
        if (omap.count(crr[i]))
        {
            noFound[i] = *omap[crr[i]].begin();
            omap[crr[i]].erase(omap[crr[i]].find(*omap[crr[i]].begin()));
            if (omap[crr[i]].size() == 0)
            {
                omap.erase(crr[i]);
            }
        }
