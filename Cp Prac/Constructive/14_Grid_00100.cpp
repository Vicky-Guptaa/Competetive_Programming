...
void solve()
{
    ...
    for (int i = 0; i < n && m > 0; i++)
    {
        ...
        for (int j = 0; j + cnt < n && m-- > 0; j++)
        {
            matrix[cnt + j][j] = 1;
        }
        cnt--;
    }
    ll rMax = 0, rMin = 1e12, cMax = 0, cMin = 1e12;
    fl(i, n)
    {
        ll oCount = 0;
        fl(j, n)
        ...
        rMax = max(rMax, oCount);
        rMin = min(rMin, oCount);
        oCount = 0;
        fl(j, n)
        {
            oCount += (matrix[i][j] & 1);
        }
        cMin = min(cMin, oCount);
        cMax = max(cMax, oCount);
    }
    ll ans = (cMin - cMax) * (cMin - cMax) + (rMin - rMax) * (rMin - rMax);
    cout << ans << "\n";
    fl(i, n)
    {
        fl(j, n)
