...
ll calc(string s)
{
    int bc = 0, wc = 0, strt = 1, maxWindow = 1;
    fl(i, s.size() - 1)
    {
        if (s[i] != s[i + 1])
        {
        ...
        {
            maxWindow = max(maxWindow, strt);
            strt = 1;
        }
    }
    return max(maxWindow, strt);
}
...
void solve()
{
    ...
    while (s.front() != s.back())
    {
        ...
        reverse(s.begin(), s.begin() + strt + 1);
        reverse(s.begin() + strt + 1, s.end());
        if (strt == n - 1)
            break;
    }
