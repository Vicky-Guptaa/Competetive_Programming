...
bool isValid(vll &arr, ll mid, ll m, ll k, ll &tempAns, vll &tempArr)
{
    ll size = 0, sum = 0, it1 = 0;
    fl(i, arr.size())
    {
        sum += arr[i];
        size++;
...
void solve()
{
    ...
    vll arr(n);
    cin >> arr;
    ll high = accumulate(vr(arr), 0ll), low = 0;
    ll res = 0;
    vll resArr(k - 1);
    ...
    while (low <= high)
    {
        ...
        ll tempAns = 0;
        vll tempArr(k - 1);
        if (isValid(arr, mid, m, k, tempAns, tempArr))
        {
            if (res < tempAns)
            {
                res = tempAns;
