...
void solve()
{
    ...
    fl(i, 1, n)
    {
        total += max(0ll, arr[i].first - arr[i - 1].second);
    }
    total += max(0ll, arr[0].first - arr[n - 1].second);

    ll ans = 1e18;
    fl(i, 1, n)
    {
        ans = min(total + arr[i].first - max(0ll, arr[i].first - arr[i - 1].second), ans);
    }
    ans = min(ans, total + arr[0].first - max(0ll, arr[0].first - arr[n - 1].second));
    cout
        << ans << '\n';
}
/*
