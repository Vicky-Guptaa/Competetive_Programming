...
void solve()
{
    ...
    cin >> la >> ra >> lb >> rb;
    set<int> taken;
    priority_queue<pll> pque1, pque2, pque;
    fl(i, la - 1, ra)
    {
        pque1.push({arr[i], i});
    }
    ...
    while (taken.size() != n)
    {
        if (!pque1.empty())
        {
            pll ele = pque1.top();
            pque1.pop();
            while (!pque1.empty() && taken.count(ele.second))
            {
                ele = pque1.top();
                pque1.pop();
            }
            if (!taken.count(ele.second))
            {
                taken.insert(ele.second);
                prof += ele.first;
            }
        }
        ...
        if (!pque2.empty())
        {
            pll ele = pque2.top();
            pque2.pop();
            while (!pque2.empty() && taken.count(ele.second))
            {
                ele = pque2.top();
                pque2.pop();
            }
