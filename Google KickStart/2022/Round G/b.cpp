...
void solve()
{
    ...
    for (auto x : arr)
    {
        if (sqrt(x.first * x.first + x.second * (lld)x.second) <= h + s)
        {
            inArr.push_back(x);
        }
    }
    ...
    if (!inBrr.empty())
        for (auto x : inArr)
        {
            if (inBrr[0].first * inBrr[0].first + inBrr[0].second * inBrr[0].second > x.first * x.first + x.second * x.second)
            {
                ascore++;
            }
        }
    ...
    if (!inArr.empty())
        for (auto x : inBrr)
        {
            if (inArr[0].first * inArr[0].first + inArr[0].second * inArr[0].second > x.first * x.first + x.second * x.second)
            {
                bscore++;
            }
        }
