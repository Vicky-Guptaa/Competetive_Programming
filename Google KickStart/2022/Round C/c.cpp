...
void solve()
{
    ll n, l;
    cin >> n >> l;
    vector<pair<int, pll>> arr, helper;
    fl(i, 0, n)
    {
        ll d, l;
        cin >> d >> l;
        arr.push_back({d, {l, i + 1}});
    }
    sort(vr(arr));
    vpll result;
    for (int i = 0; i < arr.size(); i++)
    {
        ...
        {
            if (helper.empty())
                result.push_back({arr[i].second.second, arr[i].first});
            else
            {
                ll second = arr[i].first;
                arr[i].first = helper.back().first;
                arr[i].second.first = 1;
                i--;
                helper.back().first = second;
                for (int i = helper.size() - 1; i >= 1; i--)
                {
                    helper[i].first = helper[i - 1].first;
                }
                result.push_back({helper[0].second.second, second});
        ...
        else
        {
            ...
            // result.push_back({i + 1, arr[i].first});
        }
    }
    fl(i, 0, helper.size())
    {
        result.push_back({helper[i].second.second, l - helper[i].first});
    }
    // fl(i, 0, n)
    // {
    //     cout << result[i].first << " " << result[i].second << " \n";
    // }
    sort(vr(result), mycomp);
    fl(i, 0, result.size())
    {
        cout << result[i].first << " ";
    }
