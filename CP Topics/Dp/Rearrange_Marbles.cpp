...
int helper(int n1, int n2, int k1, int k2, bool turn, vector<vector<vi>> &dp)
{
    if (n1 + n2 == 0)
        return 1;
    ...
    if (turn)
    {
        fl(i, 1, min(n1, k1) + 1)
        {
            ways = (ways + helper(n1 - i, n2, k1, k2, !turn, dp)) % mod;
        }
    }
    else
    {
        fl(i, 1, min(n2, k2) + 1)
        {
            ways = (ways + helper(n1, n2 - i, k1, k2, !turn, dp)) % mod;
        }
    }
    return dp[n1][n2][turn] = ways;
}
...
void solve()
{
    ll n1, n2, k1, k2;
    cin >> n1 >> n2 >> k1 >> k2;
    vector<vector<vi>> dp(n1 + 1, vector<vi>(n2 + 1, vi(2, -1)));
    // true for first ball false for other ball
    ll ans = helper(n1, n2, k1, k2, true, dp) + helper(n1, n2, k1, k2, false, dp);
    cout << ans << endl;
}
/*
