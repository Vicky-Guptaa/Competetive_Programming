...
void dfs(int src, vector<int> list[], vector<int> &visited, vector<int> &color, bool &isTrue)
{
    visited[src] = true;
    for (auto x : list[src])
    {
        if (visited[x] == false)
        {
            color[x] = (color[src] == 1) ? 2 : 1;
            dfs(x, list, visited, color, isTrue);
        }
...
void solve()
{
    ...
    vector<int> color(n + 1, -1), visited(n + 1, false);
    bool isTrue = true;
    fl(i, 0, n)
    {
        if (visited[i] == false)
        {
            color[i] = 1;
            dfs(i, list, visited, color, isTrue);
        }
    }
    if (isTrue)
    {
