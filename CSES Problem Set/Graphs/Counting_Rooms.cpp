...
void dfs(int r, int c, vector<string> &srr, vector<vector<int>> &visited)
{
    int rows = srr.size(), cols = srr[0].size();
    visited[r][c] = 1;
    fl(i, 0, 4)
    {
        int _r = r + moveR[i], _c = c + moveC[i];
        if (_r < 0 || _r >= rows || _c < 0 || _c >= cols || srr[_r][_c] == '#' || visited[_r][_c] == true)
            continue;
        dfs(_r, _c, srr, visited);
    }
}
