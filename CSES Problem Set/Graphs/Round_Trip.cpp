...
void dfs(int par, int src, vector<int> list[], vector<int> &visited, vector<int> &parent, vector<int> &res)
{
    visited[src] = true;
    for (auto x : list[src])
    {
        ...
        {
            parent[x] = src;
            dfs(src, x, list, visited, parent, res);
        }
        else if (par != x && res.empty())
        {
            res.push_back(x);
            int prev = src;
            while (true)
...
void solve()
{
    ...
    vector<int> visited(n + 1, false), parent(n + 1, -1), res;
    fl(i, 1, n)
    {
        if (visited[i] == false)
        {
            bool isTrue = true;
            dfs(-1, i, list, visited, parent, res);
            if (!res.empty())
            {
                cout << res.size() << "\n";
                cout << res << "\n";
