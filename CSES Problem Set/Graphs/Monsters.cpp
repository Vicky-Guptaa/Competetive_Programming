...
void solve()
{
    ...
    while (!que.empty())
    {
        auto node = que.front();
        que.pop();
        if (node.first == 'A' && visited[node.second.first][node.second.second] == 'M')
            continue;

        fl(i, 0, 4)
        {
            int x = node.second.first + moveR[i], y = node.second.second + moveC[i];
            if (x >= n || y >= m || x < 0 || y < 0 || srr[x][y] == '#')
                continue;

            if (visited[x][y] == 'M')
                continue;

            if (visited[x][y] == 'A' && node.first == 'A')
                continue;

            if ((x == n - 1 || y == m - 1 || x == 0 || y == 0) && node.first == 'A')
            {
                string ans;
                dir[x][y] = moveM[i];
                while (true)
            ...
            visited[x][y] = node.first;
            if (node.first == 'A')
                dir[x][y] = moveM[i];
            que.push({node.first, {x, y}});
        }
    }
    pn
}
