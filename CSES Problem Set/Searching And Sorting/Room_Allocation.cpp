...
void solve()
{
    ...
    int it1 = 0, it2 = 0, req = 0, Max = 0;
    while (it1 < n && it2 < n)
    {
        if (arr[it1].first <= dep[it2].first)
        {
            req++;
            it1++;
        }
    ...
    it2 = 0;
    vpi res;
    map<int, int> roomAlloted;
    while (it1 < n && it2 < n)
    {
        if (arr[it1].first <= dep[it2].first)
        {
            res.push_back({arr[it1].second, que.front()});
            roomAlloted[arr[it1].second] = que.front();
            que.pop();
            it1++;
        }
        else
        {
            que.push(roomAlloted[dep[it2].second]);
            roomAlloted.erase(dep[it2].second);
            it2++;
        }
    }
    cout << Max << "\n";
