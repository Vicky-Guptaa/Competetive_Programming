...
int helper(int s, int prev, int n, int m, vi &arr, vector<vector<int>> &dp)
{
    if (s >= n)
        return 1;
    ...
    if (arr[s] != 0)
    {
        if (s != 0 && abs(arr[s] - arr[s - 1]) > 1)
            return 0;

        return helper(s + 1, 0, n, m, arr, dp);
    }
    ...
    for (int i = 1; i <= m; i++)
    {
        ...
        {
            arr[s] = i;
            ways += helper(s + 1, i, n, m, arr, dp);
            ways %= mod;
            arr[s] = 0;
        }
        else if (abs(i - arr[s - 1]) <= 1)
        {
            arr[s] = i;
            ways += helper(s + 1, i, n, m, arr, dp);
            ways %= mod;
