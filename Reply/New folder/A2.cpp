...
{
    sort(vr(deamon), [&](vll &a, vll &b)
         { return pref[a[0]].back() < pref[b[0]].back(); });
    vll res;
    fl(i, 0, deamon_cnt)
    {
        res.push_back(deamon[i][0]);
    }
    cout << scorer(res, pref, deamon, si, smax, total_turns, deamon_cnt) << "\n";
}
...
void input_func()
{
    ...
    for (int i = 0; i < deamon_cnt; i++)
    {
        ...
        vector<ll> v = {i, sLost, turns_req, sRec, frag_cnt};
        demon[i] = v;
    }
