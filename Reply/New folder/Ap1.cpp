...
ll scorer(vector<ll> &deamon_turn, vector<vector<ll>> &pref, vector<vector<ll>> &deamon, ll si, ll smax, ll total_turns, ll deamon_cnt)
{
    ll ans = 0, j = 0;
    map<int, ll> mpRec;
    for (int i = 0; i < total_turns; i++)
    {
        ...
        si += mpRec[i];
        si = min(si, smax);
        int idx = deamon_turn[j];
        if (si >= deamon[idx][0])
        {
            si -= deamon[idx][0];
            if ((!(total_turns - i) == 0 && !deamon[idx][3] == 0))
            {
                ans += pref[idx][min(total_turns - i, deamon[idx][3]) - 1];
            }
            mpRec[deamon[idx][1] + i] += deamon[idx][2];
            j++;
        }
    }
...
void solver(vector<vector<ll>> &pref, vector<vector<ll>> &deamon, ll si, ll smax, ll total_turns, ll deamon_cnt)
{
    vector<ll> ans(deamon_cnt);
    fl(i, 0, deamon_cnt)
    {
        ans[i] = i;
    }
    ...
    while (true)
    {
        ll val = scorer(ans, pref, deamon, si, smax, total_turns, deamon_cnt);
        if (currmax < val)
        {
            cout << currmax << " ";
            for (auto &i : ans)
        ...
        int indx1 = rand() % deamon_cnt;
        int indx2 = rand() % deamon_cnt;
        swap(ans[indx1], ans[indx2]);
    }
}

void input_func()
{
    ll si, smax, total_turns, deamon_cnt;
    cin >> si >> smax >> total_turns >> deamon_cnt;
    vector<vector<ll>> demon(deamon_cnt), pref(deamon_cnt);
    for (int i = 0; i < deamon_cnt; i++)
    {
        ll sLost, turns_req, sRec, frag_cnt, sum = 0;
        cin >> sLost >> turns_req >> sRec >> frag_cnt;
        ...
        vector<ll> v = {sLost, turns_req, sRec, frag_cnt};
        demon[i] = v;
    }
    solver(pref, demon, si, smax, total_turns, deamon_cnt);
}
/*
